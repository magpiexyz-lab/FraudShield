...
- MVPs relaunched mid-phase-2 (`v{N+1}` reset) carry
  `mvp_mappings.<name>.phase2_relaunch_at` in the cross config — the verdict
  then scopes denominator + numerator to relaunch-onward data (the CSV must
  include the Start date column; Step 2 enforces this when the key is set)
...
<!-- Diagnostic invariants inherited from phase-1 (self-contained does NOT mean
     ...
     money-leak 3-way deliverability wording), and ledger-backed stalled
     triage (Step 7 annotate_stalled against
     experiment/phase2-decision-ledger.jsonl written by state-x5a; phase-1
     sibling: state-x3 — streak/escalation unlock ~5 weeks after the
     ledger's first write, run-identity clock via current_run_id). -->
...
How to export (~30 seconds):
  ...
  3. Make sure the columns include at minimum: Campaign, Clicks
     ...
     (relaunch scoping: Start date becomes HARD-REQUIRED once any MVP has
      phase2_relaunch_at configured — the validate gate rejects the CSV
      without it, because the cut cannot classify undated campaigns)
...
python3 .claude/scripts/lib/iterate_cross_ga.py validate-csv \
  ...
  --config experiment/iterate-cross-config.yaml || exit 1

# Consume-time fingerprint for the x5a runs-archive step (same invariant as
# x0a's phase-1 stamp): context is gate-readable → canonical writer only.
...
Relaunch scoping: `phase2_relaunch_at` MVPs get their pre-cut events excluded
here (the cut also heals `first_seen`, which feeds the wiring flag). The
orphan query CANNOT be cut — its rows have no `project_name` to match on.
...
from iterate_cross_ga import _load_phase2_relaunch_map
from iterate_cross_posthog_batch import paginate_discovery_query
from iterate_cross_relaunch import posthog_per_mvp_relaunch_clause
...
relaunch_map = _load_phase2_relaunch_map("experiment/iterate-cross-config.yaml")
relaunch_clause, relaunch_values = posthog_per_mvp_relaunch_clause(relaunch_map)
values = {"empty": "", "phase_campaign": phase_filter, **relaunch_values}
...
`(canonical, __orphan_<host>__)` pair sharing a `match_key` (or paired via a
`ga_campaign_aliases` entry when the direct keys differ, #2073), query the paid
...
from iterate_cross_ga import _load_aliases
...
# ga_campaign_aliases fallback pairs renamed/handover MVPs whose project_name
# does not flatten to the deploy host prefix (#2073).
aliases = _load_aliases("experiment/iterate-cross-config.yaml")

pairs = build_orphan_pairs(ctx.get("mvps", []), aliases=aliases)
phase_clause = (
    "AND properties.utm_campaign IS NOT NULL "
    "AND toString(properties.utm_campaign) LIKE {phase_campaign} "
)
...
# pay_intent EVER per MVP — deliberately NO gclid, NO phase filter, and NO
# phase2_relaunch_at cut, so a dev test or a fresh dayzero-probe proves the
# wiring. Fixed 365d lookback.
liveness_sql = (
...
The merge also detects multiple surviving `v{N}` campaign versions in one
MVP's denominator (an unrecorded relaunch, usually) and stamps
`phase2_version_conflict`; Step 7 surfaces it as a high sanity flag whose
message carries the paste-ready `phase2_relaunch_at` config line.
...
and write Phase 2 artifacts only. Operator-dropped MVPs
(`phase2.exclude_mvps`) are filtered here — verdict/report/team message only,
never the ctx or DB triage. The verdict ladder includes the zero-intent
futility rule (0 pay-intents + wiring proven + clicks >= ceil(3/θ₂) →
early NO_GO, rule of three) and its success twin, the early pass (#2152:
DB-trusted pay-intents >= ceil(θ₂ × floor) at 0 < clicks < floor → GO with
`metrics.early_pass = "worst_case_floor_bound"` + a `pay_intent_early_pass`
info flag — the rate clears θ₂ even if every remaining click converts at 0%,
so further spend buys no information; PH-sourced numerators wait for the
floor). After building scores, run the stalled triage
against the **phase2 decision ledger** (`experiment/phase2-decision-ledger.jsonl`,
written by state-x5a; raw-name keyed so the `prev_ledger.get(score["name"])`
lookup matches by construction). Run-over-run deltas, streak carry, and
escalation are REACHABLE, not immediately active: the age gate needs
`campaign_age_days` (Start date) or ≥21 days of `first_seen_in_ledger` age,
then `streak >= 2` sustained ≥ `stalled_escalate_days` — a ~5-week horizon
from the ledger's first write. `current_run_id` (from the run context) lets a
new run whose data clock froze (stopped campaigns hold `max(last_seen)` flat)
still advance the streak; same-run re-runs stay idempotent. First-ever run
(missing ledger file) degrades to the old ledger-free behavior. The cause
legend lives in state-x3.
...
# Single stamping point for phase2_relaunch_at: covers discovery rows,
# pay-intent-only rows (Step 4 auto-create), and ga_only rows (Step 5 merge)
# alike, so the verdict passthrough + stalled relaunch protection see it.
from iterate_cross_ga import _load_phase2_relaunch_map
relaunch_map = _load_phase2_relaunch_map("experiment/iterate-cross-config.yaml")
for m in ctx.get("mvps", []):
    m["phase2_relaunch_at"] = relaunch_map.get(m.get("name"))

# Operator exclusions (phase2.exclude_mvps): dropped from the verdict, report,
# and team message ONLY — in-memory filter, ctx file untouched, so discovery
# and the DB triage stay complete and the registry VERIFY set assertions hold.
from iterate_cross_verdicts import load_phase2_exclude
excluded = load_phase2_exclude("experiment/iterate-cross-config.yaml")
verdict_mvps = [m for m in ctx.get("mvps", []) if m.get("name") not in excluded]
excluded_present = sorted(
    m.get("name") for m in ctx.get("mvps", []) if m.get("name") in excluded
)
...
# Start-date VALUE gate: the stalled age gate and channel-starved detection
# need campaign_first_date, and header-only checks pass on exports whose
# Start date cells are empty (measured 2026-07-21: present on 0/101 records).
# Zero-candidate runs are exempt (nothing to age-gate).
if verdict_mvps and not any(m.get("campaign_first_date") for m in verdict_mvps):
    print(
        "STOP: no phase2 row carries a Start date value — re-export the GA CSV "
        "with the 'Start date' column populated (Columns → Attributes → Start "
        "date), save to .runs/iterate-cross-ga-clicks.csv, and re-run "
        "/iterate --cross --phase2. Without it the stalled age gate and "
        "escalation stay silent for ~21 days of ledger age.",
        file=sys.stderr,
    )
    sys.exit(1)

# Stalled triage with the phase2 decision ledger (raw-name keyed — writer and
# this reader share the key space by construction). Streak carry + escalation
# become REACHABLE here, not immediately active: the age gate needs
# campaign_age_days (Start date) or >=21 days of first_seen_in_ledger age,
# then streak >= 2 sustained >= stalled_escalate_days — a ~5-week horizon
# from the ledger's first write. current_run_id lets a new run whose data
# clock froze (stopped campaigns) still advance the streak.
from iterate_cross_verdicts import _read_prev_ledger
prev_ledger = _read_prev_ledger("experiment/phase2-decision-ledger.jsonl")

scores = [
    compute_pay_intent_verdict(
        m, issues_by_name.get(m.get("name"), {}), thresholds, reference_now=ref
    )
    for m in verdict_mvps
]

annotate_stalled(
    scores, {**thresholds, "visitors_floor": floor2}, prev_ledger,
    reference_now=ref, current_run_id=ctx.get("run_id"),
)
...
if excluded_present:
    report.append(
        f"- Excluded (operator, phase2.exclude_mvps): {', '.join(excluded_present)}"
    )
report += [
    "",
...
diag_lines = []
for s in scores:
    rel = s.get("phase2_relaunch_at")
    if rel:
        diag_lines.append(
            f"- {s.get('name')}: [info] phase2_relaunch — verdict scoped to campaigns, "
            f"clicks, and pay-intents since {rel}; pre-relaunch data dropped "
            "(denominator, numerator, and price timeline). Wiring liveness stays full-history."
        )
...
**NEXT:** Read [state-x5a-persist-phase2-ledger.md](state-x5a-persist-phase2-ledger.md) to continue.
