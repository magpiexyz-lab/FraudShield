...
- **Write exploration trace artifact** (`.claude/exploration-trace.json`):
  ```bash
  python3 -c "
  import json
  trace = {
  ...
  json.dump(trace, open('.claude/exploration-trace.json', 'w'), indent=2)
  "
  ```
...
- `.claude/exploration-trace.json` exists with required fields (`archetype`, `archetype_constraints`, `stacks_read`, `affected_files`, `affected_imports`, `exploration_steps_completed`)
...
python3 -c "
import json, os
d = json.load(open('.claude/exploration-trace.json'))
