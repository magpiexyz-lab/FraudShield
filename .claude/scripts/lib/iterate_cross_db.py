...
def _management_api_query(project_ref: str, sql: str, token: str | None = None) -> Any:
    ...
    try:
        ...
        return {"error": f"non-json response: {r.stdout[:200]}"}
...
def query_mvp_ground_truth(
    ...
    if operator_override_table:
        ...
        if schema == "auth":
            auth = count_auth_users_in_window(project_ref, window_days, token)
            if "error" in auth:
                errors.append(f"auth.users: {auth['error']}")
            breakdown["auth.users.confirmed"] = auth["confirmed"]
            first_at = auth["first_at"]
            return {
                "db_signups": auth["confirmed"],
        ...
        for t in tables:
            if t["table"] == table_only:
                ...
                break
        result = count_signups_in_window(project_ref, table_only, ts_col, window_days, token)
        if "error" in result:
    ...
    for t in tables[:5]:  # cap at 5 tables to avoid runaway queries
        ...
        if "error" in result:
            ...
            continue
        breakdown[f"public.{t['table']}"] = result["count"]
        if result["first_at"] and (not first_at or result["first_at"] < first_at):
    ...
    if not breakdown:
        return {
            ...
            "db_first_signup_at": None,
            "db_breakdown": {},
            "errors": errors or ["no tables found"],
        }

    # Pick the table with the largest count as ground truth.
    winner_table = max(breakdown, key=lambda k: breakdown[k])
    winner_count = breakdown[winner_table]

    return {
        "db_signups": winner_count,
        "db_signups_table": winner_table,
        "db_first_signup_at": first_at,
        "db_breakdown": breakdown,
        "errors": errors or None,
    }
...
def merge_into_context(
    ...
    for mvp in ctx["mvps"]:
        ...
        gt = query_mvp_ground_truth(project_ref, window_days, override_table, token)
