...
- Unless `quality: mvp` is set in experiment.yaml: verify `stack.testing` is present. If absent: stop — "Testing framework required (production quality is the default). Add `testing: playwright` (web-app) or `testing: vitest` (service/cli) to experiment.yaml `stack` and re-run `/bootstrap`. To skip TDD and use MVP mode instead, set `quality: mvp` in experiment.yaml."
