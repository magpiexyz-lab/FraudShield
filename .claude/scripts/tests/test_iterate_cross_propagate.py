...
def test_ga_accounts_survive_x1():
    """ga_accounts (state-x0a MCC sub-account attribution) must cross the
    context → data bridge — the ads-account owner channel reads it off rows."""
    payload = build_records(
        {"mvps": [
            {"name": "__orphan_momrealm__", "owner": None, "orphan": True,
             "gclid_visitors": 17, "ga_clicks": 82,
             "ga_accounts": ["Lew's MVP Account"]},
            {"name": "plain", "owner": None, "gclid_visitors": 3, "ga_clicks": 0},
        ]},
        catalog_rows=[],
        batch_status={"complete": True},
        mvp_mappings={},
    ...
    assert by["plain"]["ga_accounts"] == []


def test_orphan_owner_backstop_covers_orphan_mapping_rows():
    """persist-owner writes owner onto the __orphan_* mapping row; the x1
    backstop must carry it onto the orphan record (x0 hardcodes None)."""
    payload = build_records(
        {"mvps": [{"name": "__orphan_momrealm__", "owner": None, "orphan": True,
                   "gclid_visitors": 17, "ga_clicks": 82}]},
        catalog_rows=[],
        batch_status={"complete": True},
        mvp_mappings={"__orphan_momrealm__": {"owner": "lew"}},
    )
    assert payload["mvps"][0]["owner"] == "lew"
