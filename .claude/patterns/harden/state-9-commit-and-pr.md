...
**Gate check:** Read `.runs/verify-report.md`. If it does not exist, STOP -- go back and run STATE 8 above. Do NOT commit without a verification report.
...
```bash
RUN_ID=$(python3 -c "import json; print(json.load(open('.runs/harden-context.json')).get('run_id', ''))" 2>/dev/null || echo "")
...
- Push and open PR using `.github/PULL_REQUEST_TEMPLATE.md` format:
  ...
  - **Checklist — Verification**: populate from `.runs/verify-report.md` contents
...
- After PR is created, delete `.runs/current-plan.md` and `.runs/verify-report.md`.
...
- Checkpoint-based resume: `.runs/current-plan.md` with YAML frontmatter enables exact resume after /clear or context overflow
...
- `.runs/current-plan.md` deleted
- `.runs/verify-report.md` deleted
