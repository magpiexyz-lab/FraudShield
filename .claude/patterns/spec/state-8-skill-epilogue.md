...
**If STATE 7 step 7c.2 already wrote `.claude/observe-result.json`** (validation
...
**POSTCONDITIONS:**
- `.claude/observe-result.json` exists with `"skill": "spec"`

**VERIFY:**
```bash
python3 -c "import json; d=json.load(open('.claude/observe-result.json')); assert d['skill']=='spec'"
```
