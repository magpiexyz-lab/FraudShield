...
- Context file: `.runs/rollback-context.json`
- Expected completed states: `[0, 1, 2, 3]` (from state-registry.json)
- This skill is analysis-only and makes no code changes

**POSTCONDITIONS:**
- `.runs/observe-result.json` exists with `"skill": "rollback"`

**VERIFY:**
```bash
python3 -c "import json; d=json.load(open('.runs/observe-result.json')); assert d['skill']=='rollback'"
```
