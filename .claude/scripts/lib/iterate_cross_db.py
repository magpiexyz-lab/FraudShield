...
def get_project_detail(project_ref: str, token: str | None = None) -> dict:
    """Raw project-detail body from GET /v1/projects/{ref} (fail-soft).

    probe_project_tombstone hits the same endpoint but discards the 200 body
    (it only classifies liveness). The census db-fingerprint channel needs the
    metadata — most importantly `created_at` — so this sibling returns the
    parsed body on 200 and `{"error": ...}` on anything else. Never raises.
    """
    token = token or _read_token()
    req = urllib.request.Request(
    ...
    try:
        ...
        return {"error": f"non-json response: {body_text[:120]}"}
    if status != 200 or not isinstance(data, dict):
        return {"error": f"http {status}: {json.dumps(data)[:120]}"}
    return data
