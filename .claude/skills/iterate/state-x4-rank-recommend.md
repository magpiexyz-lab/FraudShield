...
for s in scores:
    ...
    if plain_no_go:
        ...
        if met.get('pre_cpc_verdict') not in (None, 'NO_GO') and not s.get('cpc_exception'):
            ...
            # parse_relaunch_at returns a 'YYYY-MM-DD' STRING (not datetime) —
            # convert before date math (#1876-era bug: .tzinfo on str crashed
            # this block the moment any config set phase1_relaunch_at).
            relaunch_str = parse_relaunch_at(
                (mappings.get(s.get('name')) or {}).get('phase1_relaunch_at')
            )
            if relaunch_str is not None:
                relaunch_at = datetime.fromisoformat(relaunch_str).replace(
                    tzinfo=timezone.utc
                )
...
owner inference). Confirming such a proposal creates the mapping row.
The **ads-account channel** reads each score row's `ga_accounts` (state-x0a's
MCC sub-account stamp) against the operator's `ads_accounts:` config map:
`__orphan_*` rows — which have no repo at all — get a high-confidence proposal
when every account maps to one member (the momrealm class), and real rows
whose repo/Vercel channels came up empty get a medium-confidence proposal
instead of the fallback→operator row. Accounts mapping to different members
never auto-propose. A confirmed orphan owner flows via the x1 owner backstop
into the score row, which routes the orphan's fix-tracking bullet into the
owner's team-message block (x4b). All are
