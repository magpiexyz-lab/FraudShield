...
- Context file: `.claude/audit-context.json`
- Expected completed states: `[0, 1, 2]` (from state-registry.json)
- This skill is analysis-only and makes no code changes

**POSTCONDITIONS:**
- `.claude/observe-result.json` exists with `"skill": "audit"`

**VERIFY:**
```bash
python3 -c "import json; d=json.load(open('.claude/observe-result.json')); assert d['skill']=='audit'"
```
