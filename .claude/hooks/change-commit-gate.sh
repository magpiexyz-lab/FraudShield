...
if [[ "$BRANCH" =~ ^chore/review ]]; then
  ...
  if [[ -f "$CTX" ]]; then
    ...
    if [[ "$AT_FINAL" == "yes" && ! -f "$PROJECT_DIR/.runs/review-complete.json" ]]; then
...
if [[ "$BRANCH" =~ ^fix/ ]] && [[ -f "$PROJECT_DIR/.runs/observe-result.json" ]]; then
  exit 0
fi
...
check_skill_completion "change" "$PROJECT_DIR/.runs/change-context.json"
