...
- If `quality: production` is set in experiment.yaml:
  ...
  * If absent: stop — "Production quality requires a testing framework. Add `testing: playwright` (web-app) or `testing: vitest` (service/cli) to experiment.yaml `stack`, or remove `quality: production` for MVP mode. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add testing to experiment.yaml stack, then re-run `/change`."
