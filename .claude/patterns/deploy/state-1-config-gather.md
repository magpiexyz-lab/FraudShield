...
- **Write config artifact** (`.claude/runs/deploy-config.json`):
  ```bash
  python3 -c "
  import json
  config = {
  ...
  json.dump(config, open('.claude/runs/deploy-config.json', 'w'), indent=2)
  "
  ```
...
- `.claude/runs/deploy-config.json` exists

**VERIFY:**
```bash
test -f .claude/runs/deploy-config.json
```
