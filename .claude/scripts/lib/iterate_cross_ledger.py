...
def current_snapshot(score: dict, why: str, run_date: str,
                     run_id: str | None = None,
                     persisted_at: str | None = None) -> dict:
    m = _m(score)
    return {
        ...
        # Sub-floor early-stop sentinel (#2152) — explains a GO recorded below
        # the visitors floor (worst-case bound already cleared the bar).
        "early_pass": m.get("early_pass"),
        "db_source": score.get("db_source"),
        "monthly_price_usd": m.get("monthly_price_usd"),
        # Spend / unit-economics series — these drive kill decisions and are
        # recoverable from nowhere else once the Ads campaign is deleted
        # (Google Ads exports drop deleted campaigns). Additive: old rows
        # read back None.
        "ga_cost": m.get("ga_cost"),
        "ga_cpc_usd": m.get("ga_cpc_usd"),
        "ga_currency": m.get("ga_currency"),
        "implied_cac_usd": m.get("implied_cac_usd"),
        "cpc_payback_multiple": m.get("cpc_payback_multiple"),
        "effective_cpc_cap_usd": m.get("effective_cpc_cap_usd"),
        "cpc_unit_economics_fail": m.get("cpc_unit_economics_fail"),
        # Campaign→MVP join (ROW-level on scores, not in metrics) — the only
        # durable record linking Ads campaign names to an MVP.
        "ga_campaigns": score.get("ga_campaigns"),
        ...
        "last_run": run_date,
        # Run identity: run_date is the DATA clock (reference_now for phase 2,
        # date for phase 1); persisted_at is the WALL clock; run_id
        # disambiguates same-day runs and lets annotate_stalled treat a new
        # run with a frozen data clock as a real zero-growth observation.
        "run_id": run_id,
...
def upsert_row(existing: dict | None, score: dict, alias_index: dict, run_date: str,
               ...
               why: str, force_enrich: bool = False,
               run_id: str | None = None, persisted_at: str | None = None) -> dict:
    ...
    if not frozen:
        ...
        row["current"] = current_snapshot(score, why, run_date,
                                          run_id=run_id, persisted_at=persisted_at)
        m = _m(score)
        row["verdict_history"] = append_history(
            row.get("verdict_history"),
            # Compact metrics at the change point (what numbers drove the
            # flip). append_history dedups on verdict+why only, so the extra
            # keys never affect dedup.
...
# ---------- phase 2 (pay-intent) ledger ----------
#
# Sibling file experiment/phase2-decision-ledger.jsonl. Deliberate divergences
# from the phase-1 ledger, each load-bearing:
#   - KEY = the RAW phase2 score name (NO canonical/alias collapse): the only
#     cross-run reader is annotate_stalled's `prev_ledger.get(score["name"])`
#     raw-name lookup, and phase2 mode never runs the x0 alias merge — a
#     canonical key would silently miss aliased MVPs (muse-ai vs
#     museai-studio). `mvp_canonical` is stamped as a FIELD for retro joins.
#   - No `lifecycle_status` in the snapshot: phase2 scores don't carry it, and
#     copying the phase-1 default would stamp every row "active" forever.
#     Freeze instead keys on the operator config (mvp_mappings.<canonical>.
#     lifecycle_status == killed), locking the terminal verdict pre-teardown.
#   - `run_date` is the DATA clock (x5's reference_now = max last_seen);
#     `persisted_at`/`run_id` carry the wall clock + run identity so
#     annotate_stalled can advance streaks across runs whose data clock froze
#     (stopped campaigns — exactly the escalation target population).

def phase2_why(score: dict, thresholds: dict) -> str:
    """Mechanical reason string for terminal phase2 verdicts (constant-ish per
    state so append_history dedup keeps bounding growth)."""
    if score.get("headline_verdict") != "NO_GO":
        return ""
    m = _m(score)
    theta = (thresholds or {}).get("pay_intent_rate_go", 0.02)
    rate = m.get("pay_intent_rate")
    clicks = m.get("ga_clicks")
    if rate is None or clicks is None:
        return "pay-intent below threshold"
    return f"pay-intent rate {rate:.2%} < {theta:.2%} at {clicks} clicks"


def phase2_current_snapshot(score: dict, ctx_mvp: dict | None, why: str,
                            run_date: str, run_id: str | None,
                            persisted_at: str | None) -> dict:
    m = _m(score)
    ctx_mvp = ctx_mvp or {}
    return {
        "verdict": score.get("headline_verdict"),
        "why": why,
        # Reader contract — annotate_stalled reads prev_cur["ga_clicks"],
        # ["last_run"], ["stalled_*"], ["run_id"]: names must match verbatim.
        "ga_clicks": m.get("ga_clicks") or 0,
        "pay_intents": m.get("pay_intents"),
        "pay_intent_rate": m.get("pay_intent_rate"),
        "revenue_intent_per_click": m.get("revenue_intent_per_click"),
        "pay_intent_source": m.get("pay_intent_source"),
        # Sub-floor early-stop sentinel (#2152) — explains a GO recorded at
        # ga_clicks < the 300 floor; batch readers must not re-derive INSUF.
        "early_pass": m.get("early_pass"),
        "stalled_bucket": m.get("stalled_bucket"),
        "stalled_cause": m.get("stalled_cause"),
        "stalled_since": m.get("stalled_since"),
        "stalled_streak": m.get("stalled_streak"),
        "stalled_escalated": m.get("stalled_escalated"),
        "phase2_ads_all_stopped": score.get("phase2_ads_all_stopped"),
        # Spend / campaign-join fields live ONLY in the phase2 context rows
        # (compute_pay_intent_verdict's metrics carry none). Whitelist
        # projection; the *_phase2 slices are structurally EMPTY under
        # --phase-filter runs — never read them.
        "ga_cost": ctx_mvp.get("ga_cost"),
        "ga_cpc": ctx_mvp.get("ga_cpc"),
        "ga_currency": ctx_mvp.get("ga_currency"),
        "ga_conv": ctx_mvp.get("ga_conv"),
        "ga_impressions": (ctx_mvp.get("ga_impressions")
                           if ctx_mvp.get("ga_impressions") is not None
                           else m.get("ga_impressions")),
        "ga_campaigns": ctx_mvp.get("ga_campaigns"),
        "last_run": run_date,
        "run_id": run_id,
        "persisted_at": persisted_at,
    }
...
def upsert_phase2_row(existing: dict | None, score: dict, alias_index: dict,
                      run_date: str, why: str, run_id: str | None,
                      persisted_at: str | None, ctx_mvp: dict | None,
                      lifecycle_status: str | None = None) -> dict:
    """Non-destructive phase2 upsert. Frozen rows (operator-confirmed kill)
    keep their terminal snapshot untouched."""
    name = score.get("name")
    row = dict(existing or {})
    row["mvp"] = name
    row["mvp_canonical"] = alias_index.get(name, name)
    row["phase"] = "phase-2"
    ...
    if existing and existing.get("archived_at"):
        return row  # frozen: terminal pre-teardown verdict stays locked

    row["current"] = phase2_current_snapshot(
        score, ctx_mvp, why, run_date, run_id, persisted_at)
    m = _m(score)
    row["verdict_history"] = append_history(
        row.get("verdict_history"),
        {"date": run_date, "verdict": score.get("headline_verdict"), "why": why,
    ...
    # Freeze AFTER snapshotting so the locked state is the pre-teardown one.
    if lifecycle_status == "killed" and not row.get("archived_at"):
        row["archived_at"] = run_date
    return row


def assert_no_alias_key_collision(keys: set[str], alias_index: dict) -> None:
    """A raw key that is an alias whose canonical is ALSO a key means the same
    MVP is present under two identities — merge aliases upstream first."""
    for k in sorted(keys):
        canon = alias_index.get(k)
        if canon and canon != k and canon in keys:
            raise ValueError(
                f"phase2 ledger key collision: {k!r} is an alias of {canon!r} "
                f"and both are present — merge the rows upstream before persisting"
            )
...
def cmd_persist(scores_path: str, data_path: str, ledger_path: str, input_path: str,
                proposals_path: str, now_iso: str | None = None, config_path: str | None = None,
...
def cmd_persist_phase2(scores_path: str, ctx_path: str, ledger_path: str,
                       config_path: str, now_iso: str | None = None,
                       run_id: str | None = None) -> int:
    """Phase2 sibling of cmd_persist. `--now` MUST be x5's reference_now (the
    data clock persisted into phase2-scores.json) so the writer's run_date and
    annotate_stalled's reference_now share one clock domain."""
    persisted_at = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    score_doc = json.load(open(scores_path))
    scores = score_doc.get("mvps", [])
    thresholds = score_doc.get("thresholds") or {}
    if not now_iso:
        now_iso = score_doc.get("reference_now")
    run_date = (now_iso or persisted_at)[:10]

    ctx_by_name: dict[str, dict] = {}
    if ctx_path and os.path.exists(ctx_path):
        ctx = json.load(open(ctx_path)) or {}
        for m in ctx.get("mvps", []) or []:
            if m.get("name"):
                ctx_by_name[m["name"]] = m

    config = load_yaml(config_path) if config_path else {}
    alias_index = build_alias_index(config.get("mvp_aliases"))
    mappings = config.get("mvp_mappings") or {}

    by_key = _read_ledger(ledger_path)
    new_keys = {s.get("name") for s in scores if s.get("name") and not is_orphan(s)}
    assert_no_alias_key_collision(new_keys | set(by_key), alias_index)

    upserted = 0
    frozen_n = 0
    for s in scores:
        name = s.get("name")
        if not name or is_orphan(s):
            continue  # orphans carry no durable identity — Tier-2 run-metrics keeps them
        canon = alias_index.get(name, name)
        lifecycle = (mappings.get(canon) or {}).get("lifecycle_status") \
            if isinstance(mappings.get(canon), dict) else None
        row = upsert_phase2_row(
            by_key.get(name), s, alias_index, run_date,
            why=phase2_why(s, thresholds),
            run_id=run_id, persisted_at=persisted_at,
            ctx_mvp=ctx_by_name.get(name),
    ...
    _write_ledger(ledger_path, by_key)
    print(
        f"phase2-ledger-persist: upserted {upserted} MVP(s), {frozen_n} frozen, "
        f"{len(by_key)} total rows (run_date={run_date}) → {ledger_path}"
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p2 = sub.add_parser("persist-phase2",
                        help="x5a: upsert phase2 pay-intent verdicts (raw-name keyed)")
    p2.add_argument("--scores", required=True)
    p2.add_argument("--ctx", required=True)
    p2.add_argument("--ledger", required=True)
    p2.add_argument("--config", required=True)
    p2.add_argument("--now", default=None)
    ...
    if args.cmd == "persist":
        return cmd_persist(args.scores, args.data, args.ledger, args.input, args.proposals,
                           args.now, args.config, run_id=args.run_id)
    if args.cmd == "persist-phase2":
        return cmd_persist_phase2(args.scores, args.ctx, args.ledger, args.config,
                                  now_iso=args.now, run_id=args.run_id)
    return 2
