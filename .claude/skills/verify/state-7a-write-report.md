...
**VERIFY:**
```bash
python3 -c "c=open('.runs/verify-report.md').read(); assert c.startswith('---'), 'verify-report.md missing frontmatter'; fm=c.split('---')[1] if c.count('---')>=2 else ''; missing=[f for f in ['overall_verdict:','hard_gate_failure:','process_violation:','agents_expected:','agents_completed:'] if f not in fm]; assert not missing, 'verify-report frontmatter missing: %s' % missing"
```
