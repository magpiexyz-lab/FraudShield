...
def test_roster_index_logins_aliases_emails():
    ...
    # email_aliases are indexed alongside the primary email
    idx2 = O.build_roster_index({"taran": {"github": "nukeTK",
                                           "email": "personal@gmail.com",
                                           "email_aliases": ["taran@magpiexyz.io"]}})
    assert O.map_author(None, "TARAN@magpiexyz.io", idx2) == "taran"
    assert O.map_author(None, "personal@gmail.com", idx2) == "taran"
...
# ---------- ads-account channel (GA MCC sub-account → owner) ----------

ADS_ACCOUNTS = {
    "Lew's MVP Account": "lew",
    "Radlin's MVP Account": "radlin",
    "Parth MVP": "parth",  # departed → remaps to operator
}
ROSTER_WITH_LEW = {**ROSTER, "lew": {"github": "lew-gh", "email": "lew@magpiexyz.io"}}


def _write_cfg_ads(td, mappings, ads_accounts=None, roster=None):
    try:
        import yaml
    except ImportError:
        return None
    cfg_p = os.path.join(td, "cfg.yaml")
    yaml.safe_dump({
        "mvp_mappings": mappings,
        "team_roster": roster or ROSTER_WITH_LEW,
        "ads_accounts": ads_accounts if ads_accounts is not None else ADS_ACCOUNTS,
    }, open(cfg_p, "w"))
    return cfg_p


def _write_scores(td, rows):
    p = os.path.join(td, "scores.json")
    json.dump({"mvps": rows}, open(p, "w"))
    return p


def test_load_ads_accounts_normalizes_case_and_whitespace():
    m = O.load_ads_accounts({"ads_accounts": {" Lew's MVP Account ": "lew"}})
    assert m == {"lew's mvp account": "lew"}
    assert O.load_ads_accounts({}) == {}
    assert O.load_ads_accounts({"ads_accounts": "not-a-dict"}) == {}


def test_ads_account_owner_unique_ambiguous_departed():
    ads = O.load_ads_accounts({"ads_accounts": ADS_ACCOUNTS})
    assert O.ads_account_owner(["Lew's MVP Account"], ads, ROSTER_WITH_LEW) == (
        "lew", ["Lew's MVP Account"], None
    )
    # two accounts, same member → still unique
    ads2 = O.load_ads_accounts({"ads_accounts": {"A One": "lew", "A Two": "lew"}})
    assert O.ads_account_owner(["A One", "A Two"], ads2, ROSTER_WITH_LEW)[0] == "lew"
    # ambiguous (two different members) → None
    assert O.ads_account_owner(
        ["Lew's MVP Account", "Radlin's MVP Account"], ads, ROSTER_WITH_LEW
    ) is None
    # unmapped / empty → None
    assert O.ads_account_owner(["Unknown Account"], ads, ROSTER_WITH_LEW) is None
    assert O.ads_account_owner([], ads, ROSTER_WITH_LEW) is None
    # departed member remaps to operator with provenance
    owner, accounts, original = O.ads_account_owner(["Parth MVP"], ads, ROSTER_WITH_LEW)
    assert owner == "alan" and original == "parth" and accounts == ["Parth MVP"]


def test_propose_orphan_rows_via_ads_account_channel():
    """__orphan_* score rows — unreachable by the repo channel — get a
    high-confidence ads-account proposal (the momrealm class)."""
    mappings = {
        "__orphan_momrealm__": {"signup_events": []},
        "__orphan_owned__": {"signup_events": [], "owner": "lee"},
    }
    fake = _fake_gh()
    with tempfile.TemporaryDirectory() as td:
        cfg_p = _write_cfg_ads(td, mappings)
        if cfg_p is None:
            return
        scores_p = _write_scores(td, [
            {"name": "__orphan_momrealm__", "ga_accounts": ["Lew's MVP Account"]},
            {"name": "__orphan_owned__", "ga_accounts": ["Lew's MVP Account"]},
            {"name": "__orphan_ambiguous__",
             "ga_accounts": ["Lew's MVP Account", "Radlin's MVP Account"]},
            {"name": "__orphan_unmapped__", "ga_accounts": ["Mystery Account"]},
        ])
        out = _run_propose(cfg_p, os.path.join(td, "p.json"), fake,
                           ["--scores", scores_p])
        by = {u["name"]: u for u in out["updates"]}
        assert set(by) == {"__orphan_momrealm__"}  # owned/ambiguous/unmapped skipped
        row = by["__orphan_momrealm__"]
        assert row["owner"] == "lew" and row["confidence"] == "high"
        assert row["evidence"]["resolution_method"] == "ads-account"
        assert row["evidence"]["ga_accounts"] == ["Lew's MVP Account"]
        assert "Google Ads sub-account" in row["owner_note"]


def test_propose_fallback_upgrades_to_ads_account_medium():
    """A real unowned row whose repo cannot be resolved uses the ads-account
    signal (medium) instead of falling back to the operator (echo class)."""
    mappings = {"echo": {"signup_events": []}}
    fake = _fake_gh()  # no repos resolvable
    with tempfile.TemporaryDirectory() as td:
        cfg_p = _write_cfg_ads(td, mappings)
        if cfg_p is None:
            return
        scores_p = _write_scores(td, [
            {"name": "echo", "ga_accounts": ["Radlin's MVP Account"]},
        ])
        out = _run_propose(cfg_p, os.path.join(td, "p.json"), fake,
                           ["--scores", scores_p])
        row = {u["name"]: u for u in out["updates"]}["echo"]
        assert row["owner"] == "radlin" and row["confidence"] == "medium"
        assert row["evidence"]["resolution_method"] == "ads-account"


def test_propose_no_ads_map_keeps_fallback_row():
    mappings = {"echo": {"signup_events": []}}
    fake = _fake_gh()
    with tempfile.TemporaryDirectory() as td:
        cfg_p = _write_cfg_ads(td, mappings, ads_accounts={})
        if cfg_p is None:
            return
        scores_p = _write_scores(td, [
            {"name": "echo", "ga_accounts": ["Radlin's MVP Account"]},
...
def test_propose_gh_unavailable_still_writes_orphan_ads_proposals():
    mappings = {"__orphan_momrealm__": {"signup_events": []}}
    fake = _fake_gh(auth_rc=1)
    with tempfile.TemporaryDirectory() as td:
        cfg_p = _write_cfg_ads(td, mappings)
        if cfg_p is None:
        ...
        out = _run_propose(cfg_p, os.path.join(td, "p.json"), fake,
                           ["--scores", scores_p])
        assert out["gh_unavailable"] is True
        assert [u["name"] for u in out["updates"]] == ["__orphan_momrealm__"]
        assert out["updates"][0]["owner"] == "lew"
