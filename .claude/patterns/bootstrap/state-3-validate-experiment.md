...
- If `quality: production` is set in experiment.yaml: verify `stack.testing` is present. If absent: stop — "Production quality requires a testing framework. Add `testing: playwright` (web-app) or `testing: vitest` (service/cli) to experiment.yaml `stack` and re-run `/bootstrap`. Alternatively, remove `quality: production`, bootstrap first, then use `/harden` to upgrade to production quality (it will add the testing framework automatically)."
...
- **Write validation trace artifact** (`.claude/runs/bootstrap-validation-trace.json`):
  ```bash
  python3 -c "
  import json
  trace = {
  ...
  json.dump(trace, open('.claude/runs/bootstrap-validation-trace.json', 'w'), indent=2)
  "
  ```
...
- `.claude/runs/bootstrap-validation-trace.json` exists with `experiment_valid` field

**VERIFY:**
```bash
python3 -c "import json; d=json.load(open('.claude/runs/bootstrap-validation-trace.json')); assert d.get('experiment_valid') is not None, 'experiment_valid missing'"
```
