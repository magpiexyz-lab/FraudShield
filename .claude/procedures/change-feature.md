...
- If adding `payment` to experiment.yaml `stack`: validate per `patterns/stack-dependency-validation.md` Dependency Matrix — payment requires both auth and database. Use the canonical error messages from that file's Error Message Templates section if either is missing.
