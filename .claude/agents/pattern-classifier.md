...
Read `.claude/runs/fix-log.md` from disk. Count entries matching `^\*\*Fix` pattern. If zero entries exist (only the header line), write `{"saved":0,"skipped":0,"total":0,"saved_to_files":[],"saved_to_memory":0}` to `.claude/runs/patterns-saved.json` and stop.

## Phase 1: Inventory

1. Read `.claude/runs/fix-log.md` — extract every `**Fix` entry. For each, parse: file(s) touched, symptom, cause, fix action.
