...
- Verify `stack.testing` is present in experiment.yaml. If absent: stop — "Testing framework required. Add `testing: playwright` (web-app) or `testing: vitest` (service/cli) to experiment.yaml `stack`. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add testing to experiment.yaml stack, then re-run `/change`."
