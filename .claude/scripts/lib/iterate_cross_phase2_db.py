...
from iterate_cross_relaunch import (  # noqa: E402
    parse_relaunch_at,
    postgres_lower_bound_expr,
)
...
def fetch_pay_intent_rows(
    ...
    `relaunch_at` (phase2_relaunch_at) narrows the created_at lower bound to
    the relaunch instant — the alias-qualified expr is required because both
    pay_intent and auth.users carry created_at (unqualified is ambiguous).
    """
    ...
    lower_bound = postgres_lower_bound_expr(
        "created_at", int(window_days), relaunch_at, alias="p"
    )
    sql = (
...
def fetch_gclid_no_utm_gclids(
    ...
    validity rule stays single-source. `relaunch_at` keeps this count on the
    same window basis as the numerator.
    """
    lower_bound = postgres_lower_bound_expr(
        "created_at", int(window_days), relaunch_at, alias="p"
    )
...
def _merge_railway_mvp(
    ...
    try:
        ...
        rows, reason = fetch_pay_intent_rows_railway(
            url_r["url"], window_days, phase_filter, relaunch_at=relaunch_at
        )
        if reason is not None:
            return reason
        ...
        try:
            ...
            pass
        try:
            gclids, gclid_reason = fetch_gclid_no_utm_gclids_railway(
                url_r["url"], window_days, relaunch_at=relaunch_at
            )
...
def merge_context(
    ...
    for mvp in mvps:
        ...
        # Typo-surfacing contract (same as _load_relaunch_map): a malformed
        # phase2_relaunch_at raises here, before any ctx/triage write.
        relaunch_at = parse_relaunch_at(mapping.get("phase2_relaunch_at"))
        ...
        if token is None:
            ...
            rows, reason = fetch_pay_intent_rows(
                project_ref, window_days, phase_filter, token, relaunch_at=relaunch_at
            )
            queried += 1
            ...
            if reason is None:
                ...
                # Liveness stays relaunch-UNcut (wiring proof wants any-time
                # evidence); the gclid-no-utm count shares the numerator basis.
                _stamp_diag_defaults(mvp)
                try:
                    ...
                    pass
                try:
                    gclids, gclid_reason = fetch_gclid_no_utm_gclids(
                        project_ref, window_days, token, relaunch_at=relaunch_at
                    )
                    if gclid_reason is None and gclids is not None:
                        mvp["db_gclid_no_utm_count"] = sum(1 for g in gclids if is_real_gclid(g))
                except Exception:
