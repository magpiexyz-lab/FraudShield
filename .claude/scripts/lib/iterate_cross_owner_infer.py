...
def _load_score_rows(scores_path: str | None) -> dict[str, dict]:
    """name → score/data row from an optional --scores artifact.
...
def _load_score_names(scores_path: str | None) -> list[str]:
    ...
    return list(_load_score_rows(scores_path))


# ---------- ads-account channel (GA MCC sub-account → owner) ----------
#
# Each Google Ads campaign lives in a member's MCC sub-account ("Lew's MVP
# Account"), and state-x0a stamps the distinct account names per record as
# `ga_accounts`. The operator maps account names to roster members once via
# top-level `ads_accounts:` in iterate-cross-config.yaml. This is the ONLY
# owner signal for orphan (__orphan_*) rows — no project_name means no repo to
# read commits from — and the cheap fallback for real rows whose repo channel
# came up empty (the echo class). Same proposal + operator-confirm flow as the
# commit-history channel; nothing is stamped silently.

def load_ads_accounts(config: dict) -> dict[str, str]:
    """Normalized (casefolded, stripped) account-name → roster-member map."""
    raw = config.get("ads_accounts") or {}
...
def ads_account_owner(
    ga_accounts: list, ads_map: dict[str, str], roster: dict
) -> tuple[str, list, str | None] | None:
    """Resolve a unique owner from a record's GA sub-accounts.

    Returns (owner, matched_account_names, inferred_original) — with the
    departed→operator remap already applied — or None when no account maps or
    two accounts map to DIFFERENT members (shared/ambiguous attribution never
    auto-proposes).
    """
    matched: dict[str, str] = {}
    for acct in ga_accounts or []:
        member = ads_map.get(str(acct).strip().casefold())
        if member:
            matched[str(acct)] = member
    if not matched or len(set(matched.values())) != 1:
        return None
    owner = next(iter(set(matched.values())))
    owner, inferred_original = apply_departed_remap(owner, roster)
    return owner, sorted(matched), inferred_original
...
def cmd_propose(args) -> int:
    ...
    score_rows = _load_score_rows(getattr(args, "scores", None))
    score_names = list(score_rows)
    ads_map = load_ads_accounts(config)
    ...
    def _ads_account_row(name: str, confidence: str) -> dict | None:
        """ads-account channel proposal for one row, or None (no unique map)."""
        resolved = ads_account_owner(
            (score_rows.get(name) or {}).get("ga_accounts") or [], ads_map, roster
        )
        if resolved is None:
            return None
        owner, accounts, inferred_original = resolved
        display = inferred_original or owner
        remap = f"; departed {inferred_original} → {owner}" if inferred_original else ""
        return {
            "name": name,
            "owner": owner,
            ...
            "inferred_original": inferred_original,
            "owner_note": (
                f"inferred from Google Ads sub-account {', '.join(accounts)} "
                f"→ {display}{remap} ({confidence} confidence, {today})"
            ),
            "evidence": {
                "resolution_method": "ads-account",
                "ga_accounts": accounts,
            },
        }

    def _try_ads_account(name: str, confidence: str) -> bool:
        row = _ads_account_row(name, confidence)
        if row is None:
            return False
    ...
    # Orphan channel: __orphan_* score rows are excluded from select_targets
    # (no repo to infer from) — the GA sub-account is their only owner signal.
    # Runs BEFORE the gh guard: it needs no GitHub access.
    if not args.only:
        for name in sorted(score_rows):
            if not name.startswith("__orphan_"):
                continue
            if (mappings.get(name) or {}).get("owner"):
                continue
            if _try_ads_account(name, "high"):
    ...
    if not pricing.gh_available():
        ...
        print(
            f"owner-propose: gh unavailable — wrote {inferred} ads-account "
            "proposal(s), commit-history channel skipped"
        )
        return 0
    ...
    for target in targets:
        ...
        if repo is None:
            if vc_project and _try_vercel_author(name, vc_project, vc_meta):
                continue
            # ads-account channel: cheaper than a fallback→operator row when
            # the record's GA sub-account names a unique member (echo class).
            if _try_ads_account(name, "medium"):
                continue
        ...
        if owner is None:
            ...
            elif _try_ads_account(name, "medium"):
                pass
            else:
