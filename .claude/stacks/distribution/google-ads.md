...
- **Committee-purchase products (Team Lead gate — hospitals, enterprise, government):** when the buyer is not the person clicking (typical at >= $500/mo), the standard screens measure the wrong thing: a search-ad signup is curiosity, not procurement intent, and no one self-serve-clicks "Upgrade · $500/mo" — so neither the 100-click/6% signup screen nor the Phase 2 fake-door applies to this class. Do not burn ad budget on the standard funnel for it (a bid-capped shortfall on a committee-purchase product is the usual tell that it landed in the wrong screen). Path instead:
  1. **Outreach first (free):** 10-15 conversations with real decision-makers; >= 3 agreeing to a demo/pilot carries the evidence mass of a passing Phase 1 and costs $0.
  2. **Premium demand screen (optional, Team Lead approval):** if an ads screen is still wanted, its conversion event is a demo/discovery-call booking (operator-set signup event — see `mvp_mappings.<name>.signup_events` — NOT account signup), click floor 300, θ = 2% (300 × 2% = 6 expected conversions, the same evidence mass as the standard 100 × 6%), ceiling per the escalation formula above, absolute $2,000 cap unchanged.
  3. **Value evidence** for this class is a signed pilot / LOI, judged by the Team Lead — never the fake-door.
  Tooling note: `/iterate --cross` renders fleet thresholds (100/6%); until a premium tier exists in the tooling, the Team Lead reads a premium screen's verdict manually from its own numbers and ignores the fleet-threshold row.
...
Committee-purchase products (hospitals/enterprise/gov — see Cost Model: Committee-purchase gate) never enter Phase 2: the fake-door measures self-serve willingness to pay, which does not exist for procurement purchases. Their value evidence is a signed pilot/LOI, judged by the Team Lead.

### 2. The numbers you set (STANDARDIZED: θ₂, budget cap, duration · PER-MVP: reference price)
Reference price = each MVP's own real intended price, recorded as **USD cents** in `price_cents` (UI in another currency? convert at the current rate — the cross ranking is single-currency). A simple tool and a complex product should not share one price. Standardized Phase 2 numbers:

- The reference price is **FROZEN for the entire Phase 2 run** — every pay_intent must answer the same offer, and the cross ranking multiplies your rate by this price. Changing $X mid-run corrupts the blended rate (the verdict flags it as `price_change_mid_phase`). If you must change it: tell your Team Lead, close the campaign, and start `v{N+1}` with a fresh count — old clicks and pay-intents do not carry over (your Team Lead records the relaunch date as `phase2_relaunch_at` in the cross config, which is what makes the verdict drop the pre-relaunch data).
...
1. EVENTS.yaml: add event
     pay_intent:
       ...
       properties:
         ...
         price_cents:  { type: number, required: true }   # MVP's real intended price in USD CENTS, shown not charged. If your UI displays another currency, convert at the current rate (e.g. 200 AED ≈ 5400) — the cross ranking compares revenue/click across MVPs in one currency, so a non-USD value silently corrupts it.
...
**Zero-intent early stop (rule of three):** with **0** pay-intents and wiring **proven** (day-0 probe recorded, no `pay_intent_wiring_unproven` flag), NO_GO fires already at `ceil(3/θ₂)` = **150 clicks** — the 95% CI upper bound `3/n` is then at or below θ₂, so waiting for the 300 floor buys no information on a zero numerator. The verdict row carries a `pay_intent_futility` note when this rule (not a full-floor sample) produced the NO_GO.
**Early pass (worst-case floor bound, #2152) — the success twin:** with **DB-trusted** pay-intents already at `ceil(θ₂ × 300)` = **6** while clicks are still below the floor, GO fires early — the rate clears θ₂ even if every remaining click to the floor converts at 0%, so further spend buys no information either. The row carries a `pay_intent_early_pass` info flag with the arithmetic; **action: stop the phase-2 campaign for that MVP — measurement is complete.** PostHog-sourced numerators never trigger it (they wait for the floor), and the same rule guards Phase 1 (`phase1_early_pass`: DB-trusted signups ≥ `ceil(conv_rate_go × 100)` = 6 → early GO, stop Phase-1 spend and take the promote path).
