...
def test_fetch_pay_intent_rows_relaunch_bound_is_alias_qualified(monkeypatch):
    """phase2_relaunch_at narrows the created_at bound; the alias qualifier is
    load-bearing (auth.users also has created_at — unqualified is ambiguous)."""
    calls = []
    ...
    monkeypatch.setattr(db, "_management_api_query", fake_query)
    rows, reason = db.fetch_pay_intent_rows(
        "project-ref", 365, "%phase2%", "token", relaunch_at="2026-08-01"
    )
    assert reason is None
    sql = calls[1]
    assert 'p."created_at" >= greatest(' in sql
    assert "timestamptz '2026-08-01'" in sql
    assert "now() - INTERVAL '365 days'" in sql


def test_fetch_gclid_no_utm_carries_relaunch_bound(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(db, "_management_api_query", fake_query)
    gclids, reason = db.fetch_gclid_no_utm_gclids(
        "project-ref", 365, "token", relaunch_at="2026-08-01"
    )
    assert reason is None
    assert 'p."created_at" >= greatest(' in calls[0]
    assert "timestamptz '2026-08-01'" in calls[0]
...
def test_merge_threads_phase2_relaunch_at_from_config(tmp_path, monkeypatch):
    """phase2_relaunch_at on a mapping narrows THAT MVP's numerator window;
    unmapped MVPs keep the plain rolling window. The liveness diag stays
    unfiltered either way (no INTERVAL, no greatest)."""
    context = tmp_path / "context.json"
    config = tmp_path / "config.yaml"
    triage = tmp_path / "triage.json"
    _write_json(
        context,
        {
            "phase2_run_token": "run-relaunch",
            "window_days": 365,
            "phase2_utm_campaign_like": "%phase2%",
            "mvps": [
                {"name": "neuralpost", "pay_intents": 0},
                {"name": "diarly", "pay_intents": 0},
            ],
        },
    )
    config.write_text(
        "\n".join([
            "mvp_mappings:",
            "  neuralpost:",
            "    supabase_project_ref: neural-ref",
            "    phase2_relaunch_at: '2026-08-01'",
            "  diarly:",
            "    supabase_project_ref: diarly-ref",
            "",
        ])
    )

    sqls_by_ref: dict[str, list[str]] = {}

    def fake_query(project_ref, sql, token):
        sqls_by_ref.setdefault(project_ref, []).append(sql)
        if "to_regclass" in sql:
            return [{"regclass": "pay_intent"}]
        return []
    ...
    neural_data = [s for s in sqls_by_ref["neural-ref"] if "ILIKE" in s][0]
    assert 'p."created_at" >= greatest(' in neural_data
    assert "timestamptz '2026-08-01'" in neural_data
    diarly_data = [s for s in sqls_by_ref["diarly-ref"] if "ILIKE" in s][0]
    assert "greatest(" not in diarly_data
    assert 'p."created_at" >= now() - INTERVAL' in diarly_data
    # Liveness diag: unfiltered on both, relaunch or not.
    for ref in ("neural-ref", "diarly-ref"):
        liveness = [s for s in sqls_by_ref[ref] if "max(created_at)" in s][0]
        assert "INTERVAL" not in liveness and "greatest(" not in liveness
    # gclid-no-utm shares the numerator basis.
    neural_gclid = [s for s in sqls_by_ref["neural-ref"] if "utm_campaign IS NULL" in s][0]
    assert "timestamptz '2026-08-01'" in neural_gclid


def test_merge_malformed_phase2_relaunch_at_raises(tmp_path, monkeypatch):
    """Typo-surfacing contract: a malformed date fails the merge loudly."""
    context = tmp_path / "context.json"
    config = tmp_path / "config.yaml"
    triage = tmp_path / "triage.json"
    _write_json(
        context,
        {
            "phase2_run_token": "run-typo",
            "window_days": 365,
            "phase2_utm_campaign_like": "%phase2%",
            "mvps": [{"name": "neuralpost", "pay_intents": 0}],
        },
    )
    config.write_text(
        "mvp_mappings:\n"
        "  neuralpost:\n"
        "    supabase_project_ref: neural-ref\n"
        "    phase2_relaunch_at: 'next week'\n"
    )
    monkeypatch.setattr(db, "_management_api_query", lambda *a, **k: [])
    try:
        db.merge_context(str(context), str(config), str(triage), token="token")
        raise AssertionError("expected ValueError for malformed phase2_relaunch_at")
    except ValueError:
        pass
...
def test_fetch_pay_intent_rows_railway_carries_relaunch_bound(monkeypatch):
    captured = []
    ...
    monkeypatch.setattr(db, "_psql_query", fake_psql)
    rows, reason = db.fetch_pay_intent_rows_railway(
        "postgresql://fake", 30, "%phase2%", relaunch_at="2026-08-01"
    )
    assert reason is None
    data_sql = captured[1]
    assert 'p."created_at" >= greatest(' in data_sql
    assert "timestamptz '2026-08-01'" in data_sql
