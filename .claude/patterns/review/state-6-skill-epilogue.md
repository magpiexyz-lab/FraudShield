...
```bash
rm -f .claude/observe-result.json .claude/epilogue-context.json .claude/observer-diffs.txt
```
...
**POSTCONDITIONS:**
- `.claude/observe-result.json` exists with `"skill": "review"`
- Observer agent run (if branch had diffs) or "clean" recorded

**VERIFY:**
```bash
python3 -c "import json; d=json.load(open('.claude/observe-result.json')); assert d['skill']=='review'"
```
