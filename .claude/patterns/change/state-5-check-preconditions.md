...
- If `$ARGUMENTS` mentions payment or the change will add `payment` to the stack: verify `stack.auth` and `stack.database` are present in experiment.yaml. If `stack.auth` is missing, stop: "Payment requires authentication. Add `auth: supabase` (or another auth provider) to experiment.yaml `stack` first. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add auth, then re-run `/change`." If `stack.database` is missing, stop: "Payment requires a database. Add `database: supabase` (or another database provider) to experiment.yaml `stack` first. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add database, then re-run `/change`." Then read the payment stack file's `assumes` list and verify each `category/value` pair against experiment.yaml `stack` (the value must match exactly, not just the category — e.g., `framework/nextjs` requires `stack.services[].runtime: nextjs`). If any assumption is unmet, stop: "Payment stack requires [unmet dependencies]. Current stack has [actual values]. Update experiment.yaml `stack` to match, or choose a different payment provider. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: make the required changes to experiment.yaml, then re-run `/change`."
- If `$ARGUMENTS` mentions email or the change will add `email` to the stack: verify `stack.auth` and `stack.database` are present in experiment.yaml. If `stack.auth` is missing, stop: "Email requires authentication to know who to send emails to. Add `auth: supabase` (or another auth provider) to experiment.yaml `stack` first. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add auth, then re-run `/change`." If `stack.database` is missing, stop: "Email requires a database to track user activation status. Add `database: supabase` (or another database provider) to experiment.yaml `stack` first. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add database, then re-run `/change`." Then read the email stack file's `assumes` list and verify each `category/value` pair against experiment.yaml `stack` (the value must match exactly, not just the category — e.g., `framework/nextjs` requires `stack.services[].runtime: nextjs`). If any assumption is unmet, stop: "Email stack requires [unmet dependencies]. Current stack has [actual values]. Update experiment.yaml `stack` to match, or choose a different email provider. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: make the required changes to experiment.yaml, then re-run `/change`."
...
- Unless `quality: mvp` is set in experiment.yaml:
  ...
  * If absent: stop — "Testing framework required (production quality is the default). Add `testing: playwright` (web-app) or `testing: vitest` (service/cli) to experiment.yaml `stack`. To skip TDD, set `quality: mvp` in experiment.yaml. To abort: `git checkout main && git branch -D <branch-name>`. To fix and retry: add testing to experiment.yaml stack, then re-run `/change`."
