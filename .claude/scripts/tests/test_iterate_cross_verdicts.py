...
# ---------- resolve_orphan_owners + flag hints (#2073) ----------

def _museai_orphan_score(**over):
    row = {
        "name": "__orphan_museai__",
        "owner": None,
        "headline_verdict": VERDICT_MISSING_PROJECT_NAME,
        "visitors_needed": 0,
        "orphan": True,
        ...
        "metrics": {"ga_clicks": 0, "pay_intents": 0},
    }
    row.update(over)
    return row
...
_MUSEAI_OWNERS = {"museai-studio": "lee"}


def test_resolve_orphan_owners_routes_via_sample_utm_campaign_alias():
    scores = [
        {"name": "museai-studio", "owner": "lee",
         "headline_verdict": VERDICT_INSUFFICIENT, "metrics": {}},
        _museai_orphan_score(),
    ]
    audit = resolve_orphan_owners(scores, _MUSEAI_ALIASES, _MUSEAI_OWNERS)
    orphan = scores[1]
    assert orphan["owner"] == "lee"
    assert orphan["owner_source"] == "campaign_alias"
    assert audit == [{
        "name": "__orphan_museai__", "owner": "lee",
        "owner_source": "campaign_alias",
        "via_campaign": "museai-search-phase2-v1",
    }]


def test_resolve_orphan_owners_config_mapping_precedes_campaign():
    scores = [_museai_orphan_score()]
    owners = {"__orphan_museai__": "chester", "museai-studio": "lee"}
    audit = resolve_orphan_owners(scores, _MUSEAI_ALIASES, owners)
    assert scores[0]["owner"] == "chester"
    assert scores[0]["owner_source"] == "config_mapping"
    assert audit[0]["via_campaign"] is None


def test_resolve_orphan_owners_leaves_ambiguous_and_unresolvable_unowned():
    ambiguous = _museai_orphan_score(
        ga_campaigns=["museai-search-phase2-v1", "diarly-search-phase2-v1"],
    )
    scores = [
        {"name": "museai-studio", "owner": "lee",
         "headline_verdict": VERDICT_INSUFFICIENT, "metrics": {}},
        {"name": "diarly", "owner": "lew",
         "headline_verdict": VERDICT_INSUFFICIENT, "metrics": {}},
        ambiguous,
    ]
    owners = {"museai-studio": "lee", "diarly": "lew"}
    aliases = {"museai": "museai-studio"}
    audit = resolve_orphan_owners(scores, aliases, owners)
    assert ambiguous.get("owner") is None  # two distinct owners -> no stamp
    assert audit == []

    unresolvable = _museai_orphan_score(sample_utm_campaign=None)
    resolve_orphan_owners([unresolvable], aliases, owners)
    assert unresolvable.get("owner") is None


def test_resolve_orphan_owners_ignores_owned_and_non_missing_rows():
    owned = _museai_orphan_score(owner="radlin")
    normal = {"name": "diarly", "owner": None,
              "headline_verdict": VERDICT_INSUFFICIENT,
              "sample_utm_campaign": "diarly-search-phase2-v1", "metrics": {}}
    audit = resolve_orphan_owners([owned, normal], _MUSEAI_ALIASES,
                                  {"museai-studio": "lee", "diarly": "lew"})
    assert owned["owner"] == "radlin" and "owner_source" not in owned
    assert normal["owner"] is None  # non-MISSING rows are never stamped
    assert audit == []


def test_team_message_routes_alias_resolved_orphan_with_campaign_note():
    scores = [
        {"name": "museai-studio", "owner": "lee",
         "headline_verdict": VERDICT_INSUFFICIENT, "visitors_needed": 92,
         "metrics": {"ga_clicks": 208, "pay_intents": 0}},
        _museai_orphan_score(),
    ]
    resolve_orphan_owners(scores, _MUSEAI_ALIASES, _MUSEAI_OWNERS)
    text = emit_team_message(
        scores,
        {"pay_intent_visitors_floor": 300, "pay_intent_rate_go": 0.02},
        phase=2,
    )
    assert "@lee" in text
    assert "museai (orphan)" in text            # _orphan_display, not the raw sentinel
    assert "__orphan_museai__" not in text
    assert "campaigns: museai-search-phase2-v1" in text
    assert "@unassigned" not in text


def test_flag_suffix_renders_hints_and_respects_exclusions():
    flags = [
        {"flag": "pay_intent_wiring_unproven", "severity": "high", "message": "m"},
        {"flag": "cpc_unit_economics_fail", "severity": "high", "message": "m"},
        {"flag": "price_change_mid_phase", "severity": "info", "message": "m"},
        {"flag": "some_future_flag", "severity": "high", "message": "m"},
    ]
    suffix = _flag_suffix(flags)
    assert "⚠ pay_intent_wiring_unproven (walk the deployed funnel" in suffix
    assert "some_future_flag" in suffix          # unknown flag renders name-only
    assert "some_future_flag (" not in suffix
    assert "price_change_mid_phase" not in suffix  # info severity excluded
    excluded = _flag_suffix(flags, exclude=("cpc_unit_economics_fail",))
    assert "cpc_unit_economics_fail" not in excluded
    assert _flag_suffix([]) == "" and _flag_suffix(None) == ""


def test_compute_pay_intent_verdict_copies_sample_utm_campaign():
    mvp = {
        "name": "museai-studio",
        "sample_utm_campaign": "museai-search-phase2-v1",
        "gclid_visitors": 165,
    ...
    score = compute_pay_intent_verdict(
        mvp, {}, {"visitors_floor": 300, "pay_intent_visitors_floor": 300}
    )
    assert score["sample_utm_campaign"] == "museai-search-phase2-v1"


def test_main_emit_team_message_resolves_orphan_and_leaves_scores_file_untouched():
    import io
    import json
    import tempfile
    ...
    pre_scores = {
        "thresholds": {"pay_intent_visitors_floor": 300, "pay_intent_rate_go": 0.02},
        "window_days": 365,
        "mvps": [
            {"name": "museai-studio", "owner": "lee",
             "headline_verdict": VERDICT_INSUFFICIENT, "visitors_needed": 92,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        json.dump(pre_scores, open(scores_path, "w"))
        ...
        text = out.getvalue()
        assert "@lee" in text
        assert "museai (orphan)" in text
        assert "@unassigned" not in text
        # Emit-gated resolution: the persisted scores artifact is untouched.
        assert open(scores_path).read() == before
...
def test_futility_zero_intent_rule_of_three_fires():
    """0 intents + wiring proven + clicks >= ceil(3/θ₂)=150 → early NO_GO."""
    score = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=160, pay_intents=0,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
    ...
    assert score["headline_verdict"] == VERDICT_NO_GO
    assert score["metrics"]["futility"] == "zero_intent_rule_of_three"
    flag = {f["flag"]: f for f in score["tracking_sanity_flags"]}["pay_intent_futility"]
    assert flag["severity"] == "info"
    assert "95% CI upper bound" in flag["message"]


def test_futility_blocked_by_wiring_unproven():
    """Any wiring_unproven flag (even high) keeps the row INSUF — a zero
    numerator may be a broken pay wall, not absent demand."""
    mvp = pay_mvp(
        ga_clicks=160, pay_intents=0,
        # Last pay_intent EVER predates the campaign's first_seen on both
        # sides → wiring unproven (the _wiring_mvp shape).
        first_seen="2026-06-29T03:35:03Z",
        ph_last_pay_intent_any_at="2026-06-15T03:29:57Z",
        db_last_pay_intent_at="2026-06-15 03:29:57.001721+00:00",
    )
    score = compute_pay_intent_verdict(mvp, {}, dict(THRESHOLDS), reference_now=_S_REF)
    assert any(
        f["flag"] == "pay_intent_wiring_unproven" for f in score["tracking_sanity_flags"]
    )
    assert score["headline_verdict"] == VERDICT_INSUFFICIENT
    assert score["metrics"]["futility"] is None


def test_futility_boundaries_stay_insufficient():
    # Below the futility threshold: 149 clicks, 0 intents → INSUF.
    below = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=149, pay_intents=0,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
                first_seen="2026-07-20T00:00:00Z"),
        {},
        dict(THRESHOLDS),
    ...
    assert below["headline_verdict"] == VERDICT_INSUFFICIENT
    # Non-zero numerator never takes the futility path.
    nonzero = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=200, pay_intents=1,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
                first_seen="2026-07-20T00:00:00Z"),
        {},
        dict(THRESHOLDS),
    ...
    assert nonzero["headline_verdict"] == VERDICT_INSUFFICIENT
    assert nonzero["metrics"]["futility"] is None


def test_futility_threshold_tracks_theta_and_override():
    # θ=0.01 → derived futility clicks = 300: 200 clicks stays INSUF.
    theta_low = dict(THRESHOLDS, pay_intent_rate_go=0.01)
    insuf = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=200, pay_intents=0,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
                first_seen="2026-07-20T00:00:00Z"),
        {},
        theta_low,
    ...
    assert insuf["headline_verdict"] == VERDICT_INSUFFICIENT
    # Explicit pay_intent_futility_clicks overrides the derived value.
    explicit = dict(THRESHOLDS, pay_intent_futility_clicks=100)
    early = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=110, pay_intents=0,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
                first_seen="2026-07-20T00:00:00Z"),
        {},
        explicit,
    ...
    assert early["headline_verdict"] == VERDICT_NO_GO
    assert early["metrics"]["futility"] == "zero_intent_rule_of_three"


# ---------- Early-pass (worst-case floor bound, #2152) ----------


def _db_pay_mvp(ga_clicks, pay_intents, **extra):
    """Trusted-DB pay-intent shape: the early pass only accepts DB numerators."""
    return pay_mvp(
        ga_clicks=ga_clicks,
        pay_intents=pay_intents,
        db_pay_intents_paid=pay_intents,
        db_pay_intents_raw=pay_intents,
        db_pay_intents_real_windowed=True,
        db_pay_intent_source="supabase",
        **extra,
    )
...
def test_early_pass_phase2_fires_below_floor():
    """8 DB-trusted intents @ 120 clicks: ceil(2% x 300) = 6 already cleared →
    GO with the sentinel + info flag. The futility rule's success twin."""
    score = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=120, pay_intents=8),
        {},
        dict(THRESHOLDS),
        reference_now=_S_REF,
    )
    assert score["headline_verdict"] == VERDICT_GO
    assert score["metrics"]["early_pass"] == "worst_case_floor_bound"
    assert score["metrics"]["pay_intent_source"] == "db_real"
    assert score["visitors_needed"] == 0
    flag = {f["flag"]: f for f in score["tracking_sanity_flags"]}["pay_intent_early_pass"]
    assert flag["severity"] == "info"
    assert "ceil(2% x 300) = 6" in flag["message"]
    # Guaranteed-rate invariant: intents >= ceil(θ₂ x floor) at sub-floor
    # clicks implies the at-floor rate bar is already met.
    assert score["metrics"]["pay_intent_rate"] >= THRESHOLDS["pay_intent_rate_go"]


def test_early_pass_phase2_boundary():
    # Exactly the derived threshold (6 @ 299) fires; one below stays INSUF.
    at = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=299, pay_intents=6), {}, dict(THRESHOLDS), reference_now=_S_REF
    )
    assert at["headline_verdict"] == VERDICT_GO
    assert at["metrics"]["early_pass"] == "worst_case_floor_bound"
    under = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=299, pay_intents=5), {}, dict(THRESHOLDS), reference_now=_S_REF
    )
    assert under["headline_verdict"] == VERDICT_INSUFFICIENT
    assert under["metrics"]["early_pass"] is None
    assert under["visitors_needed"] == 1


def test_early_pass_phase2_requires_db_trusted_numerator_and_clicks():
    """The early exit demands stronger evidence than the at-floor verdict:
    PH-sourced numerators wait for the floor, and organic intents with zero
    paid clicks never fire it."""
    ph_sourced = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=120, pay_intents=8), {}, dict(THRESHOLDS), reference_now=_S_REF
    )
    assert ph_sourced["metrics"]["pay_intent_source"] == "ph"
    assert ph_sourced["headline_verdict"] == VERDICT_INSUFFICIENT
    assert ph_sourced["metrics"]["early_pass"] is None
    zero_clicks = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=0, pay_intents=9), {}, dict(THRESHOLDS), reference_now=_S_REF
    )
    assert zero_clicks["headline_verdict"] == VERDICT_INSUFFICIENT
    assert zero_clicks["metrics"]["early_pass"] is None


def test_early_pass_phase2_at_floor_uses_normal_path():
    # 6 @ 300: the sub-floor branch is not taken — plain at-floor GO, no sentinel.
    score = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=300, pay_intents=6), {}, dict(THRESHOLDS), reference_now=_S_REF
    )
    assert score["headline_verdict"] == VERDICT_GO
    assert score["metrics"]["early_pass"] is None
    assert not any(
        f["flag"] == "pay_intent_early_pass" for f in score["tracking_sanity_flags"]
    )


def test_early_pass_phase2_threshold_tracks_theta_and_floor():
    # θ₂=0.04 → ceil(0.04 x 300) = 12: 11 intents stays INSUF, 12 fires.
    theta_high = dict(THRESHOLDS, pay_intent_rate_go=0.04)
    insuf = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=150, pay_intents=11), {}, theta_high, reference_now=_S_REF
    )
    assert insuf["headline_verdict"] == VERDICT_INSUFFICIENT
    assert insuf["metrics"]["early_pass"] is None
    fired = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=150, pay_intents=12), {}, theta_high, reference_now=_S_REF
    )
    assert fired["headline_verdict"] == VERDICT_GO
    assert fired["metrics"]["early_pass"] == "worst_case_floor_bound"
    # Floor override tracks too: floor=200 → ceil(2% x 200) = 4.
    low_floor = dict(THRESHOLDS, pay_intent_visitors_floor=200)
    fired2 = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=120, pay_intents=4), {}, low_floor, reference_now=_S_REF
    )
    assert fired2["headline_verdict"] == VERDICT_GO
    assert fired2["metrics"]["early_pass"] == "worst_case_floor_bound"


def test_early_pass_phase2_futility_and_rung_precedence_undisturbed():
    # Zero-intent futility fires exactly as before (early pass is the sibling
    # branch, mutually exclusive on the numerator).
    fut = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=160, pay_intents=0,
                ph_last_pay_intent_any_at="2026-07-21T00:00:00Z",
                first_seen="2026-07-20T00:00:00Z"),
        {},
    ...
    assert fut["headline_verdict"] == VERDICT_NO_GO
    assert fut["metrics"]["futility"] == "zero_intent_rule_of_three"
    assert fut["metrics"]["early_pass"] is None
    # Data-integrity rungs outrank the early pass.
    blocked = compute_pay_intent_verdict(
        _db_pay_mvp(ga_clicks=120, pay_intents=8),
        {"missing_project_name": True},
        dict(THRESHOLDS),
        reference_now=_S_REF,
    )
    assert blocked["headline_verdict"] == VERDICT_MISSING_PROJECT_NAME
    assert blocked["metrics"]["early_pass"] is None


def test_early_pass_phase1_fires_fraudshield_shape():
    """46 clicks / 23 trusted-DB signups (the motivating case): GO + sentinel."""
    record = mvp(name="fraudshield", visitors=60, signups=20,
                 signup_events=["activate"], ga_clicks=46)
    record.update({"db_signups": 23, "db_signups_real": 23,
                   "db_signups_real_windowed": True, "db_source": "supabase"})
    score = compute_headline_verdict(record, {}, THRESHOLDS)
    assert score["headline_verdict"] == VERDICT_GO
    assert score["metrics"]["early_pass"] == "worst_case_floor_bound"
    assert score["visitors_needed"] == 0
    flag = {f["flag"]: f for f in score["tracking_sanity_flags"]}["phase1_early_pass"]
    assert flag["severity"] == "info"
    assert "ceil(6% x 100) = 6" in flag["message"]
    assert score["metrics"]["signup_source"] == "db_real"


def _db_p1_mvp(visitors, signups, ga_clicks=0, **extra):
    """Phase-1 shape with a trusted-DB numerator (windowed + supabase)."""
    record = mvp(visitors=visitors, signups=0, ga_clicks=ga_clicks)
    record.update({
        "db_signups": signups,
        "db_signups_real": signups,
        "db_signups_real_windowed": True,
        "db_source": "supabase",
    })
    record.update(extra)
...
def test_early_pass_phase1_boundary():
    # ceil(6% x 100) = 6: exactly 6 fires, 5 stays INSUF with the full gap.
    six = compute_headline_verdict(_db_p1_mvp(0, 6, ga_clicks=50), {}, THRESHOLDS)
    assert six["headline_verdict"] == VERDICT_GO
    assert six["metrics"]["early_pass"] == "worst_case_floor_bound"
    five = compute_headline_verdict(_db_p1_mvp(0, 5, ga_clicks=50), {}, THRESHOLDS)
    assert five["headline_verdict"] == VERDICT_INSUFFICIENT
    assert five["metrics"]["early_pass"] is None
    assert five["visitors_needed"] == 50


def test_early_pass_phase1_requires_db_trusted_numerator():
    """PH-only numerators never take the early exit — a small-sample PH spike
    may be an attribution glitch (the 8-visitor/6-signup dead-link case)."""
    score = compute_headline_verdict(mvp(visitors=40, signups=7), {}, THRESHOLDS)
    assert score["metrics"]["signup_source"] == "ph"
    assert score["headline_verdict"] == VERDICT_INSUFFICIENT
    assert score["metrics"]["early_pass"] is None


def test_early_pass_phase1_cpc_economics_still_overrides():
    """An early GO is subject to the same unit-economics override as an at-floor
    GO: pre_cpc_verdict preserves the GO and the sentinel survives the flip."""
    m = _econ_mvp(monthly_price=50, ga_clicks=60, signups=7,
                  db_signups_real_windowed=True, db_source="supabase")
    score = _econ_verdict(m)
    assert score["metrics"]["early_pass"] == "worst_case_floor_bound"
    assert score["metrics"]["pre_cpc_verdict"] == VERDICT_GO
    assert score["headline_verdict"] == VERDICT_NO_GO
    assert score["metrics"]["cpc_unit_economics_fail"] is True


def test_early_pass_phase1_ph_denominator_with_db_numerator():
    # ga_clicks=0 → PH gclid_visitors denominator; a trusted-DB numerator still
    # fires (denominator choice mirrors the at-floor rules; the trust gate is
    # about the NUMERATOR source).
    score = compute_headline_verdict(_db_p1_mvp(40, 7), {}, THRESHOLDS)
    assert score["headline_verdict"] == VERDICT_GO
    assert score["metrics"]["early_pass"] == "worst_case_floor_bound"
    assert score["metrics"]["denominator_source"] == "ph"


def test_load_phase2_exclude(tmp_path):
    cfg = tmp_path / "cfg.yaml"
    cfg.write_text("phase2:\n  exclude_mvps:\n    - stylica-ai\n    - '  padded  '\n")
    assert verdicts.load_phase2_exclude(str(cfg)) == {"stylica-ai", "padded"}
    cfg.write_text("phase2:\n  utm_campaign_like: '%phase2%'\n")
    assert verdicts.load_phase2_exclude(str(cfg)) == set()
    assert verdicts.load_phase2_exclude(str(tmp_path / "missing.yaml")) == set()
    cfg.write_text("phase2:\n  exclude_mvps: not-a-list\n")
    assert verdicts.load_phase2_exclude(str(cfg)) == set()


def test_version_conflict_flag_carries_paste_ready_config_line():
    score = compute_pay_intent_verdict(
        pay_mvp(
            name="neuralpost",
            phase2_version_conflict={
                "versions": [
                    {"campaign": "neuralpost-search-phase2-v1", "version": 1,
                     "start_date": "2026-06-29", "clicks": 126},
                    {"campaign": "neuralpost-search-phase2-v2", "version": 2,
                     "start_date": "2026-08-01", "clicks": 30},
                ],
                "proposed_relaunch_at": "2026-08-01",
            },
        ),
        {},
        dict(THRESHOLDS),
    ...
    flags = {f["flag"]: f for f in score["tracking_sanity_flags"]}
    flag = flags["phase2_version_conflict"]
    assert flag["severity"] == "high"
    assert 'mvp_mappings.neuralpost.phase2_relaunch_at: "2026-08-01"' in flag["message"]
    assert "2 phase2 campaign versions" in flag["message"]
    assert "126 clicks" in flag["message"]


def test_version_conflict_flag_absent_without_field():
    score = compute_pay_intent_verdict(pay_mvp(), {}, dict(THRESHOLDS), reference_now=_S_REF)
    assert not any(
        f.get("flag") == "phase2_version_conflict"
        for f in score["tracking_sanity_flags"]
    )
    # Missing proposal degrades to a re-export instruction, never a bare date.
    score2 = compute_pay_intent_verdict(
        pay_mvp(phase2_version_conflict={
            "versions": [
                {"campaign": "a-phase2-v1", "version": 1, "start_date": "2026-07-01", "clicks": 5},
                {"campaign": "a-phase2-v2", "version": 2, "start_date": None, "clicks": 3},
            ],
            "proposed_relaunch_at": None,
        }),
        {},
        dict(THRESHOLDS),
        reference_now=_S_REF,
    )
    flag = {f["flag"]: f for f in score2["tracking_sanity_flags"]}["phase2_version_conflict"]
    assert "Start date column" in flag["message"]
    assert "phase2_relaunch_at:" not in flag["message"]


def test_pay_intent_verdict_passes_through_phase2_relaunch_at():
    """Sibling of compute_headline_verdict's phase1_relaunch_at passthrough —
    without it, annotate_stalled's relaunch protection is inert in x5."""
    score = compute_pay_intent_verdict(
        pay_mvp(phase2_relaunch_at="2026-08-01"),
        {},
        dict(THRESHOLDS),
        reference_now=_S_REF,
    )
    assert score["phase2_relaunch_at"] == "2026-08-01"
    absent = compute_pay_intent_verdict(pay_mvp(), {}, dict(THRESHOLDS), reference_now=_S_REF)
    assert absent["phase2_relaunch_at"] is None


def test_annotate_stalled_phase2_relaunch_protection_suppresses():
    """A fresh phase-2 relaunch suppresses the stalled bucket for
    relaunch_protection_days (phase-2 analogue of the phase-1 test above)."""
    # _S_REF is the reference instant; a relaunch 2 days before it is inside
    # the default 30-day protection window.
    score = compute_pay_intent_verdict(
        pay_mvp(ga_clicks=0, campaign_first_date="2026-06-01", ga_impressions=0,
                phase2_relaunch_at="2026-07-20"),
        {},
        dict(THRESHOLDS),
        reference_now=_S_REF,
    )
...
def test_team_message_routes_owned_orphan_fix_tracking_bullet():
    """An orphan row with an owner (ads-account attribution, confirmed) gets a
    fix-missing-project_name bullet in the owner's block; unowned orphans stay
    excluded (momrealm class)."""
    owned = {
        "name": "__orphan_momrealm__", "owner": "lew",
        "headline_verdict": VERDICT_MISSING_PROJECT_NAME,
        "visitors_needed": 0,
        "ga_campaigns": ["momrealm-search-v1"],
        "metrics": {"gclid_visitors": 17, "ga_clicks": 82, "signups": 0},
    }
    unowned = {
        "name": "__orphan_mystery__", "owner": None,
        "headline_verdict": VERDICT_MISSING_PROJECT_NAME,
        "visitors_needed": 0, "metrics": {"gclid_visitors": 9, "signups": 0},
    }
    text = emit_team_message([owned, unowned], TEAM_THRESHOLDS)
    assert "@lew" in text
    assert "momrealm (orphan): fix missing project_name tracking" in text
    assert "momrealm-search-v1" in text
    assert "__orphan_momrealm__" not in text  # sentinel never leaks verbatim
    assert "__orphan_mystery__" not in text and "mystery" not in text
