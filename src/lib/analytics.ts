...
export function track(event: string, properties?: Record<string, unknown>) {
  ...
  // Test marker is a pure in-memory sessionStorage write with no PostHog dependency.
  // Write it BEFORE the misconfigured short-circuit so e2e/funnel assertions work in
  // local/CI environments without a real POSTHOG_KEY (behavior-verifier finding #3).
