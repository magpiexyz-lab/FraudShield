...
- `.runs/current-plan.md` exists with approved module list
...
For each approved CRITICAL module **in dependency order, sequentially**:
  ...
  g. Update checkpoint in `.runs/current-plan.md` frontmatter to `step3-module-[next]` (where [next] is the 1-indexed number of the next module to process)

- **Write modules trace artifact** (`.runs/harden-modules-trace.json`):
  ```bash
  python3 -c "
  import json
  trace = {
  ...
  json.dump(trace, open('.runs/harden-modules-trace.json', 'w'), indent=2)
  "
  ```
...
- `.runs/harden-modules-trace.json` exists

**VERIFY:**
```bash
test -f .runs/harden-modules-trace.json
```
