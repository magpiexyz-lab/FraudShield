...
Verdict precedence (first match wins):
  ...
  3. INSUFFICIENT_DATA       (visitors < visitors_floor — not enough sample;
                              early-pass escape: DB-trusted signups >= ceil(
                              conv_rate_go x visitors_floor) with visitors > 0
                              -> GO with metrics.early_pass — the worst-case
                              bound already clears the bar, #2152)
...
DEFAULT_CONFIG = {
    ...
    "thresholds": {
        ...
        # Phase-2 zero-intent futility (rule of three): with 0 pay-intents and
        # wiring PROVEN, NO_GO fires already at ceil(3 / pay_intent_rate_go)
        # clicks (default 150 at θ₂=0.02) — the 95% CI upper bound 3/n is then
        # <= θ₂, so waiting for the 300 floor buys no information on a zero
        # numerator. Set pay_intent_futility_clicks explicitly to override the
        # derived value; wiring-unproven rows NEVER take this path.
        "pay_intent_futility_clicks": None,
    },
    "window_days": 90,
...
def annotate_stalled(
    ...
    phase1_relaunch_at / phase2_relaunch_at within relaunch_protection_days
    (fresh flight); age below channel_starve_min_days, or unknown on BOTH
    sources (no GA Start date AND no prior ledger row — first-ever sighting).
    ...
    Run-identity override (phase 2): when BOTH `current_run_id` and the prev
    row's `current.run_id` are present and DIFFER, a delta_days <= 0
    observation is a REAL zero-growth reading, not a same-run re-persist —
    phase2's reference clock is max(last_seen), which freezes the moment a
    campaign stops serving (exactly the escalation target population), so
    without this the streak could never advance on stopped campaigns. Same
    run_id keeps the verbatim-carry idempotence. Phase-1 callers pass no
    current_run_id and keep the pre-existing behavior bit-for-bit.
    ...
    for score in scores:
        ...
        if _active_override(score.get("channel_waiver"), ref) is not None:
            continue
        relaunch = _parse_iso_datetime(
            score.get("phase1_relaunch_at") or score.get("phase2_relaunch_at")
        )
        ...
        prev_run_id = prev_cur.get("run_id")
        distinct_run = bool(
            current_run_id and prev_run_id and prev_run_id != current_run_id
        )
        ...
        if prev_clicks is not None and prev_last_run is not None and ref is not None:
            ...
            if delta_days <= 0 and not distinct_run:
        ...
        zero_growth = (
            click_delta == 0
            and delta_days is not None
            and (delta_days > 0 or distinct_run)
        )
...
def compute_version_conflict_flag(mvp: dict) -> dict | None:
    """Flag multiple phase2 campaign versions sharing one denominator.

    Reads `phase2_version_conflict` (stamped by the GA merge on surviving —
    i.e. post-relaunch-drop — campaigns only, so a correctly configured
    phase2_relaunch_at self-silences this). Two+ distinct v{N} versions with
    no cut usually means a v{N+1} relaunch whose cut nobody recorded: the
    verdict is silently blending the dead flight with the re-test. The
    message carries the exact paste-ready config line (date = the newest
    version's CSV Start date) — detection is automatic, the decision to
    invalidate v{N} stays with the operator.
    """
    conflict = mvp.get("phase2_version_conflict")
    if not isinstance(conflict, dict) or not conflict.get("versions"):
        return None
    rows = conflict["versions"]
    name = mvp.get("name") or "(unknown)"
    n_versions = len({r.get("version") for r in rows})
    top = max(r.get("version") or 0 for r in rows)
    summary = "; ".join(
        f"v{r.get('version')} start {r.get('start_date') or '—'} · {r.get('clicks')} clicks"
        for r in rows
    )
    proposal = conflict.get("proposed_relaunch_at")
    if proposal:
        remedy = (
            f"if v{top} is a relaunch, add to experiment/iterate-cross-config.yaml: "
            f'mvp_mappings.{name}.phase2_relaunch_at: "{proposal}" and re-run; '
            "if the versions are deliberate parallel campaigns, ignore this flag."
        )
    else:
        remedy = (
            f"v{top}'s Start date is missing from the CSV — re-export with the "
            "Start date column to get the paste-ready proposal."
        )
    return {
        "flag": "phase2_version_conflict",
        "severity": "high",
        "message": (
            f"{n_versions} phase2 campaign versions share this denominator "
            f"({summary}) with no phase2_relaunch_at cut — the verdict is "
            f"blending them. {remedy}"
        ),
    }
...
def load_phase2_exclude(path: str | None) -> set[str]:
    """Read `phase2.exclude_mvps` (operator-dropped MVPs) from the config.

    Excluded names are filtered from the x5 verdict/report/team message ONLY
    (state-x5 Step 7, in-memory). Discovery and the DB triage still include
    them by design — the VERIFY triage==ctx set assertion stays intact, and
    the operator's drop decision lives visibly in config, not in data loss.
    """
    if not path or not os.path.exists(path):
        return set()
    try:
    ...
    raw = (cfg.get("phase2") or {}).get("exclude_mvps") or []
...
def compute_headline_verdict(
    ...
    Precedence (first match wins):
      ...
      4. visitors < visitors_floor → INSUFFICIENT_DATA (not enough sample),
         UNLESS DB-trusted signups >= ceil(conv_rate_go x visitors_floor) and
         visitors > 0 → GO with metrics.early_pass = "worst_case_floor_bound"
         (#2152: the rate clears the bar even if every remaining click to the
         floor converts at 0%; PH-sourced numerators wait for the floor)
    ...
    # Early-pass threshold (worst-case floor bound, #2152): derived, no config
    # key — ceil(bar x floor) signups guarantee conv_rate >= conv_rate_go even
    # if every remaining click to the floor converts at 0%. (signups_go in the
    # config is a vestigial twin of this number; deliberately not read here.)
    early_pass_signups = (
        math.ceil(conv_rate_go * visitors_floor) if conv_rate_go > 0 else None
    )
    early_pass = None
    ...
    if issues.get("missing_project_name"):
        ...
        # Early pass (worst-case floor bound, #2152): enough signups that the
        # rate clears conv_rate_go even with zero further conversions through
        # the floor — buying the rest of the sample adds no information. The
        # success twin of phase-2's zero-intent futility escape, and like it,
        # the early exit demands STRONGER evidence than the at-floor verdict:
        # only a DB-trusted numerator qualifies (PostHog fallbacks wait for
        # the floor — the 8-visitor/6-signup dead-link incident is exactly a
        # small-sample PH spike from broken attribution) and the paid
        # denominator must be nonzero (organic signups with no phase-1 clicks
        # prove nothing about the paid funnel).
        if (
            early_pass_signups is not None
            and signups >= early_pass_signups
            and visitors > 0
            and (signup_source or "").startswith("db_")
        ):
            verdict = VERDICT_GO
    ...
    if early_pass:
        sanity_flags = sanity_flags + [{
            "flag": "phase1_early_pass",
            "severity": "info",
            "message": (
                f"GO issued early at {visitors} visitors: {signups} signups >= "
                f"ceil({_format_rate_pct(conv_rate_go)} x {visitors_floor}) = "
                f"{early_pass_signups} — conversion clears the "
                f"{_format_rate_pct(conv_rate_go)} GO gate even if every "
                f"remaining click to the {visitors_floor}-visitor floor converts "
                "at 0%. Phase-1 measurement complete; stop Phase-1 spend and "
                "take the promote path."
            ),
        }]
    ...
    return {
        ...
        "metrics": {
            ...
            # Non-None ⇔ the (pre-CPC) GO came from the early-pass worst-case
            # bound at sub-floor visitors (see phase1_early_pass flag).
        ...
        # PostHog-observed campaign sample — the emit-time orphan-owner
        # resolver's fallback signal when ga_campaigns is empty (#2073).
        "sample_utm_campaign": mvp.get("sample_utm_campaign"),
        # GA sub-account attribution (state-x0a) — the ads-account owner
        # channel reads it from score rows via owner-infer --scores.
        "ga_accounts": mvp.get("ga_accounts") or [],
...
def compute_pay_intent_verdict(
    ...
    version_flag = compute_version_conflict_flag(mvp)
    if version_flag:
        tracking_sanity_flags.append(version_flag)
    ...
    # Zero-intent futility threshold (rule of three): explicit config wins,
    # else derived so it tracks θ₂ (0.02 → 150). Never applies while wiring
    # is unproven — a zero numerator may be a broken pay wall, not no demand.
    futility_clicks = thresholds.get("pay_intent_futility_clicks") or (
        math.ceil(3 / pay_intent_rate_go) if pay_intent_rate_go > 0 else None
    )
    futility = None
    # Early-pass threshold (worst-case floor bound, #2152): derived, no config
    # key — ceil(θ₂ × floor) pay-intents guarantee the rate clears the GO gate
    # even if every remaining click to the floor converts at 0%. The success
    # twin of the zero-intent futility escape below; its evidence gate (DB-
    # trusted numerator, nonzero denominator) lives at the branch. Wiring is
    # structurally proven for it (compute_pay_intent_wiring_flag returns None
    # when pay_intents > 0).
    early_pass_intents = (
        math.ceil(pay_intent_rate_go * visitors_floor)
        if pay_intent_rate_go > 0
        else None
    )
    early_pass = None
    if issues.get("missing_project_name"):
        ...
        # Early exits demand STRONGER evidence than at-floor verdicts (the
        # floor itself is dilution protection): futility requires wiring
        # proven; the early pass requires a DB-trusted numerator (PostHog
        # fallbacks wait for the floor — a small-sample PH spike may be an
        # attribution glitch, cf. the phase-1 dead-link case) and a nonzero
        # paid denominator (organic intents with zero clicks prove nothing
        # about the paid funnel).
        if (
            early_pass_intents is not None
            and pay_intents >= early_pass_intents
            and ga_clicks > 0
            and pay_intent_source.startswith("db_")
        ):
            verdict = VERDICT_GO
            early_pass = "worst_case_floor_bound"
            tracking_sanity_flags.append({
                "flag": "pay_intent_early_pass",
                "severity": "info",
                "message": (
                    f"GO issued early at {ga_clicks} clicks: {pay_intents} "
                    f"pay-intents >= ceil({_format_rate_pct(pay_intent_rate_go)}"
                    f" x {visitors_floor}) = {early_pass_intents} — the rate "
                    f"clears the {_format_rate_pct(pay_intent_rate_go)} GO gate "
                    f"even if every remaining click to the {visitors_floor}-click "
                    "floor converts at 0%. Phase-2 measurement complete; stop "
                    "phase-2 spend for this MVP."
                ),
            })
        elif (
            pay_intents == 0
            and wiring_flag is None
            and futility_clicks is not None
            and ga_clicks >= futility_clicks
        ):
            verdict = VERDICT_NO_GO
            futility = "zero_intent_rule_of_three"
            tracking_sanity_flags.append({
                "flag": "pay_intent_futility",
                "severity": "info",
                "message": (
                    f"NO_GO issued early at {ga_clicks} clicks: 0 pay-intents "
                    f"puts the 95% CI upper bound (3/{ga_clicks} = "
                    f"{3 / ga_clicks:.2%}) at or below the {pay_intent_rate_go:.2%} "
                    f"GO gate — waiting for the {visitors_floor}-click floor buys "
                    "no information on a zero numerator. Wiring is proven, so "
                    "this is demand, not breakage."
                ),
            })
        else:
            verdict = VERDICT_INSUFFICIENT
    ...
    return {
        ...
        "metrics": {
            ...
            "ga_impressions": mvp.get("ga_impressions"),
            # Non-None ⇔ the NO_GO came from the zero-intent early rule, not
            # a full-floor sample (see pay_intent_futility flag for the math).
            "futility": futility,
            # Non-None ⇔ the GO came from the early-pass worst-case bound at
            # sub-floor clicks (see pay_intent_early_pass flag for the math).
            "early_pass": early_pass,
        },
        ...
        # Passthrough (cf. compute_headline_verdict's phase1_relaunch_at):
        # annotate_stalled's relaunch protection reads it off the score.
        "phase2_relaunch_at": mvp.get("phase2_relaunch_at"),
    }
...
PAY_INTENT_ACTION_TEMPLATES = {
    ...
    VERDICT_MISSING_PROJECT_NAME: "Fix {name} tracking: Phase 2 paid events arrived without `project_name`. Check `src/lib/analytics.ts` PROJECT_NAME on the deploy the ad's Final URL points at.",
...
# One short imperative fix hint per high-severity sanity flag; rendered next
# to the flag name in team-message ⚠ suffixes so owners get "what to do", not
# just a code (#2073). Flags without an entry render name-only.
FLAG_HINTS = {
    "pay_intent_wiring_unproven": "walk the deployed funnel to the paywall once",
    "phase2_ads_stopped": "resume the campaign or close the verdict",
    "foreign_campaign_traffic": "check ad Final URLs and sitelinks",
    "pay_intent_ph_exceeds_db": "check the server-side pay_intent DB write",
    "pay_intent_db_zero_with_ph": "check the server-side pay_intent DB write",
    "pay_intent_unattributed_rows": "check utm passthrough into the DB insert",
    "phase2_version_conflict": "record phase2_relaunch_at in the cross config",
    "ph_attribution_broken": "check the project_name super-property registration",
    "ph_overcount": "dedupe client signup events against the DB",
    "late_instrumentation": "tracking went live after launch; early data lost",
    "db_zero_with_ph_signups": "check the server-side signup DB write",
    "cpc_over_cap": "lower bids or record a cpc_exception",
    "channel_starved": "raise bids/budget or accept the channel verdict",
}


def _flag_suffix(flags: list[dict] | None, exclude: tuple[str, ...] = ()) -> str:
    """Shared ⚠ suffix for team-message bullets: high-severity flag names,
    each with its FLAG_HINTS fix hint when one exists."""
    high = [
        tf["flag"]
        for tf in (flags or [])
        if tf.get("severity") == "high" and tf.get("flag") not in exclude
    ]
    if not high:
        return ""
    rendered = [f"{f} ({FLAG_HINTS[f]})" if f in FLAG_HINTS else f for f in high]
    return f" ⚠ {', '.join(rendered)}"


def _orphan_display(name: str) -> str:
    """Human name for an __orphan_<host>__ sentinel row (host + marker)."""
    if name.startswith("__orphan_") and name.endswith("__"):
        return f"{name[len('__orphan_'):-len('__')]} (orphan)"
    return name


def resolve_orphan_owners(
    scores: list[dict],
    aliases: dict[str, str],
    owner_by_mvp: dict[str, str],
) -> list[dict]:
    """Route unowned MISSING_PROJECT_NAME rows to an owner (#2073). Mutates
    `scores` in place; returns an audit list of the stamps applied.

    Resolution order per unowned orphan row:
      1. `mvp_mappings.__orphan_<host>__.owner` from the config (the
         operator-confirmed ads-account channel; phase-1 propagate honors it
         but the x5 ctx build hardcodes orphan owner to None) →
         owner_source "config_mapping".
      2. The row's campaigns (`ga_campaigns`, else `sample_utm_campaign`)
         through `bucket_campaign` + `ga_campaign_aliases` to an MVP whose
         `mvp_mappings` owner is set → owner_source "campaign_alias". The
         stamp is applied only when exactly one distinct owner resolves.

    Callers gate this to team-message emission — persisted scores artifacts
    (x3 --output) must never carry emit-time owner stamps.
    """
    audit: list[dict] = []
    real_names = {
        str(s.get("name"))
        for s in scores
        if s.get("name") and not str(s.get("name")).startswith("__")
    }
    for s in scores:
        if s.get("headline_verdict") != VERDICT_MISSING_PROJECT_NAME or s.get("owner"):
            continue
        name = str(s.get("name") or "")
        owner = (owner_by_mvp or {}).get(name)
        source = "config_mapping" if owner else None
        via = None
        if not owner:
            # Lazy import: ga imports classify only, so verdicts→ga is acyclic.
            from iterate_cross_ga import bucket_campaign
            candidates = [c for c in (s.get("ga_campaigns") or []) if c]
            if not candidates and s.get("sample_utm_campaign"):
                candidates = [s.get("sample_utm_campaign")]
            resolved: dict[str, str] = {}
            for campaign in candidates:
                mvp_name, _reason = bucket_campaign(
                    str(campaign), real_names, aliases or {}
                )
                cand_owner = (owner_by_mvp or {}).get(mvp_name) if mvp_name else None
                if cand_owner:
                    resolved.setdefault(cand_owner, str(campaign))
            if len(resolved) == 1:
                owner, via = next(iter(resolved.items()))
                source = "campaign_alias"
        if owner:
            s["owner"] = owner
            s["owner_source"] = source
            audit.append({
                "name": name,
                "owner": owner,
                "owner_source": source,
...
def _team_phase1_bullet(score: dict, visitors_floor: int, conv_rate_go: float) -> str:
    ...
    elif verdict == VERDICT_MISSING_PROJECT_NAME:
        # Reachable only for OWNED orphan rows (ads-account attribution) —
        # unowned orphans are skipped by emit_team_message and stay in the
        # report. The bullet names the campaigns so the owner can find the
        # deploy the Final URL points at.
        name = _orphan_display(name)
        campaigns = ", ".join(score.get("ga_campaigns") or []) or "no GA campaign matched"
        text = (
            "fix missing project_name tracking — paid events arrive with no "
            f"`project_name` (campaigns: {campaigns}); check src/lib/analytics.ts "
            "PROJECT_NAME on the deploy the ad's Final URL points at"
        )
    ...
    suffix = _flag_suffix(
        score.get("tracking_sanity_flags"), exclude=("cpc_unit_economics_fail",)
    )
...
def emit_team_message(
    ...
    (operator decision 2026-07-27). Action items only — UNOWNED orphan
    ...
    owners with nothing to do are omitted entirely. An owned orphan row gets a
    fix-tracking bullet in its owner's block; ownership comes from ads-account
    attribution (`ads_accounts` config, operator-confirmed) or from the
    emit-time `resolve_orphan_owners` pass (config `__orphan_<host>__` mapping
    or campaign→`ga_campaign_aliases`→`mvp_mappings.owner`, #2073) that the
    CLI runs before calling this function.
    """
    if phase == 2:
        visitors_floor = int(
    ...
    for s in sort_scores_by_owner(scores):
        if s.get("headline_verdict") == VERDICT_MISSING_PROJECT_NAME and not s.get("owner"):
            # Unowned orphan streams live in the report, not the team message.
            # An OWNED orphan (ads-account attribution, operator-confirmed) is
            # routable — its fix-tracking bullet lands in the owner's block.
            continue
        ...
        if phase == 2:
            ...
            if s.get("headline_verdict") == VERDICT_MISSING_PROJECT_NAME:
                # Owned orphan: render host (orphan) like phase 1 does, and
                # name the campaigns so the owner can find the deploy (#2073).
                display_name = _orphan_display(name)
                campaigns = [c for c in (s.get("ga_campaigns") or []) if c]
                if not campaigns and s.get("sample_utm_campaign"):
                    campaigns = [s.get("sample_utm_campaign")]
                if campaigns:
                    campaign_note = f" (campaigns: {', '.join(campaigns)})"
            ...
            suffix = _flag_suffix(s.get("tracking_sanity_flags"))
            _add(owner, f"{action}{campaign_note}{suffix}")
        else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.emit_team_message:
        # Emit-gated on purpose: --output invocations (x3) must never leak
        # emit-time owner stamps into the persisted scores artifact (#2073).
        # _load_aliases is used (not load_config) because load_config drops
        # keys outside DEFAULT_CONFIG — ga_campaign_aliases among them.
        from iterate_cross_ga import _load_aliases, _load_owner_map
        resolve_orphan_owners(
            scores, _load_aliases(args.config), _load_owner_map(args.config)
        )
        obligations = None
