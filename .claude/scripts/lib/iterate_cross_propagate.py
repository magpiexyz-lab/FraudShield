...
def build_records(
    ...
    for m in ctx.get("mvps", []):
        ...
        rec = {
            ...
            # GA sub-account attribution (state-x0a merge). The MCC sub-account
            # names the launching member — the ads-account owner channel
            # (iterate_cross_owner_infer + config `ads_accounts`) reads it off
            # score rows, so it must survive the context → data bridge.
            "ga_accounts": m.get("ga_accounts") or [],
