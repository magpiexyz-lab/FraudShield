...
18. (web-app only) **No asChild** — grep entire `src/` directory for `asChild` EXCLUDING `src/components/ui/` and `src/components/magicui/` (auto-generated by shadcn CLI). BLOCK if any match found in user-authored files, listing file:line for each.
