...
- **Write targeting artifact** (`.runs/distribute-targeting.json`):
  ```bash
  python3 -c "
  import json
  targeting = {
  ...
  json.dump(targeting, open('.runs/distribute-targeting.json', 'w'), indent=2)
  "
  ```

**VERIFY:**
```bash
test -f .runs/distribute-targeting.json
```
