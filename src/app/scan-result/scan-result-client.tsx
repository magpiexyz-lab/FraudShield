...
// Demo / fallback scan — rendered when no row resolves (DEMO_MODE single-row reads
// return null per supabase.md). A bare score is forbidden (guardrail #7), so the
// fixture always carries a full forensic signal breakdown.
const FALLBACK_SCAN: ScansRow = {
  id: "demo-scan",
  user_id: "demo-user-id",
  doc_type: "pay_stub",
  fraud_score: 78,
  file_meta: {
    filename: "paystub_march_2026.pdf",
    mime: "application/pdf",
    size: 184_320,
    pdf_producer: "Adobe Photoshop 25.0",
    pdf_creator: "Canva",
    pdf_created: "2026-03-11T09:42:00Z",
    pdf_modified: "2026-03-11T09:58:00Z",
    page_count: 1,
  },
  signals: [
    {
      ...
      severity: "fraud",
      detail:
        "PDF producer is “Adobe Photoshop” — pay stubs are generated by payroll software, never image editors. Strong sign of manual fabrication.",
      weight: 34,
    },
    {
      id: "META-04",
      label: "Created and modified minutes apart",
      severity: "suspect",
      detail:
        "Creation and last-modified timestamps are 16 minutes apart on the same day, consistent with a freshly edited template rather than an issued document.",
      weight: 18,
    },
    {
      id: "TMPL-09",
      label: "Known fraud-template match",
      severity: "fraud",
      detail:
        "Layout matches a circulating editable pay-stub template (signature THX-2231) in the FraudShield template database.",
      weight: 22,
    },
    {
      id: "XDOC-02",
      label: "Net pay reconciles to gross",
      severity: "clear",
      detail:
        "Gross minus listed deductions equals net pay to the cent — the arithmetic is internally consistent.",
      weight: -4,
    },
  ],
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const resolved = (data as ScansRow | null) ?? FALLBACK_SCAN;
        setScan(resolved);
        setScansUsed(typeof count === "number" ? count : 1);
        setState("ready");
      } catch {
        if (cancelled) return;
        // Never dead-end the activation payoff — fall back to the demo scan.
