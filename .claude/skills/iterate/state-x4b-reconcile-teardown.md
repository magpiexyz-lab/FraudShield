...
are omitted by design; UNOWNED orphan MISSING_PROJECT_NAME rows and
archived/promoted reference rows stay in the report only. An orphan row with
a confirmed owner (ads-account channel, state-x4) DOES get a
fix-missing-project_name bullet in its owner's block — and before emitting,
the CLI's `resolve_orphan_owners` pass also routes orphans whose owner is
derivable from the config `__orphan_<host>__` mapping or from the row's
campaigns via `ga_campaign_aliases` → `mvp_mappings.owner` (#2073).
...
**NEXT:** Read [state-x4c-infra-census.md](state-x4c-infra-census.md) to continue.
