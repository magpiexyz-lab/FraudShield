...
- When adding or modifying a state's VERIFY section, update BOTH the state file and `state-registry.json` — they must stay in sync. Run `.claude/scripts/verify-linter.sh` to detect drift. States with `"true"` VERIFY must include a `<!-- VERIFY=true: <reason> -->` comment in the state file.
