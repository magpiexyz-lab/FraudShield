...
def test_load_aliases_normalizes_keys_and_drops_falsy_values():
    """_load_aliases: keys match_key-normalized, values verbatim; missing file → {}."""
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(cfg, "w") as f:
            f.write(
                ...
                "  stay-lica-ai: stylica-ai\n"
                "  dropped: null\n"
            )
        assert _load_aliases(cfg) == {
            "museai": "museai-studio",
            "staylicaai": "stylica-ai",
        }
        assert _load_aliases(os.path.join(td, "absent.yaml")) == {}
...
def test_main_merge_phase_filter_skips_relaunch_map():
    """The x5 --phase-filter merge must ignore phase1_relaunch_at: the phase-1
    relaunch cut is a different axis and never scopes the phase2 denominator
    (phase2_relaunch_at is the phase-2 axis, covered by the tests below)."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        ctx_path = os.path.join(td, "context.json")
        cfg_path = os.path.join(td, "config.yaml")
...
def test_main_merge_phase_filter_applies_phase2_relaunch_map():
    """phase2_relaunch_at drops a relaunched MVP's pre-cut phase2 campaigns
    from the denominator while other MVPs stay untouched."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        ctx_path = os.path.join(td, "context.json")
        cfg_path = os.path.join(td, "config.yaml")
        ...
        with open(ga_csv_path, "w", encoding="utf-8") as f:
            ...
            f.write("neuralpost-search-phase2-v1,126,2026-06-29\n")
            f.write("neuralpost-search-phase2-v2,30,2026-08-01\n")
            f.write("diarly-search-phase2-v1,99,2026-07-23\n")
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write(
                "mvp_mappings:\n"
                "  neuralpost:\n    phase2_relaunch_at: '2026-08-01'\n"
            )
        json.dump(
            {"mvps": [_mvp("neuralpost"), _mvp("diarly")], "mode": "cross-phase2"},
            open(ctx_path, "w"),
        )
        rc = main([
            "merge",
        ...
        ctx = json.load(open(ctx_path))
        by = {m["name"]: m for m in ctx["mvps"]}
        assert by["neuralpost"]["ga_clicks"] == 30
        assert by["neuralpost"]["ga_campaigns"] == ["neuralpost-search-phase2-v2"]
        assert by["neuralpost"]["campaign_first_date"] == "2026-08-01"
        assert by["diarly"]["ga_clicks"] == 99
        unmatched = json.load(open(unmatched_path))
        assert [u["name"] for u in unmatched if u.get("reason") == "pre-relaunch"] == [
            "neuralpost-search-phase2-v1"
        ]


def test_main_merge_phase2_relaunch_ignored_without_phase_filter():
    """The phase-2 relaunch axis never leaks into the phase-1 merge: without
    --phase-filter, phase2_relaunch_at is not even loaded."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        ctx_path = os.path.join(td, "context.json")
        cfg_path = os.path.join(td, "config.yaml")
        ...
        with open(ga_csv_path, "w", encoding="utf-8") as f:
            f.write("Campaign,Clicks,Start date\n")
            f.write("neuralpost-search-v1,80,2026-06-01\n")
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write(
                "mvp_mappings:\n"
                "  neuralpost:\n    phase2_relaunch_at: '2026-08-01'\n"
            )
        json.dump({"mvps": [_mvp("neuralpost")]}, open(ctx_path, "w"))
        rc = main([
            "merge",
            "--ga-csv", ga_csv_path,
            "--context", ctx_path,
        ...
        ctx = json.load(open(ctx_path))
        by = {m["name"]: m for m in ctx["mvps"]}
        assert by["neuralpost"]["ga_clicks"] == 80
        unmatched = json.load(open(unmatched_path))
        assert [u for u in unmatched if u.get("reason") == "pre-relaunch"] == []


def test_main_merge_flags_phase2_version_conflict():
    """Two surviving v{N} versions with no cut → paste-ready conflict payload
    (versions ascending, proposal = the newest version's Start date)."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        ctx_path = os.path.join(td, "context.json")
        cfg_path = os.path.join(td, "config.yaml")
        ...
        ctx = json.load(open(ctx_path))
        by = {m["name"]: m for m in ctx["mvps"]}
        conflict = by["neuralpost"]["phase2_version_conflict"]
        assert conflict is not None
        assert conflict["proposed_relaunch_at"] == "2026-08-01"
        assert [r["version"] for r in conflict["versions"]] == [1, 2]
        assert conflict["versions"][0]["clicks"] == 126
        assert by["diarly"]["phase2_version_conflict"] is None


def test_main_merge_version_conflict_silenced_by_configured_cut():
    """A correctly configured phase2_relaunch_at drops v1 pre-merge, so only
    one version survives and no conflict is stamped."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        ctx_path = os.path.join(td, "context.json")
        cfg_path = os.path.join(td, "config.yaml")
        ...
        json.dump({"mvps": [_mvp("neuralpost")], "mode": "cross-phase2"}, open(ctx_path, "w"))
        rc = main([
            "merge",
            "--ga-csv", ga_csv_path,
            "--context", ctx_path,
        ...
        ctx = json.load(open(ctx_path))
        by = {m["name"]: m for m in ctx["mvps"]}
        assert by["neuralpost"]["phase2_version_conflict"] is None
        assert by["neuralpost"]["ga_clicks"] == 30


def test_merge_version_conflict_numeric_sort_v10():
    """v10 outranks v2 numerically (lexicographic would invert them)."""
    campaigns = [
        {"name": "app-search-phase2-v2", "clicks": 50, "conv": 0.0,
         "start_date": "2026-07-01"},
        {"name": "app-search-phase2-v10", "clicks": 20, "conv": 0.0,
         "start_date": "2026-08-15"},
    ]
    merged, _ = merge_ga_clicks(
        campaigns, [_mvp("app")], flag_version_conflicts=True
    )
    conflict = merged[0]["phase2_version_conflict"]
    assert conflict["proposed_relaunch_at"] == "2026-08-15"
    assert [r["version"] for r in conflict["versions"]] == [2, 10]


def test_merge_version_conflict_gated_off_by_default():
    """Phase-1 merges (no kwarg) never stamp the field non-None even for
    multi-version campaign sets."""
    campaigns = [
        {"name": "app-search-v1", "clicks": 50, "conv": 0.0, "start_date": "2026-06-01"},
        {"name": "app-search-v2", "clicks": 20, "conv": 0.0, "start_date": "2026-07-01"},
    ]
    merged, _ = merge_ga_clicks(campaigns, [_mvp("app")])
    assert merged[0]["phase2_version_conflict"] is None


def test_merge_version_conflict_missing_start_date_no_proposal():
    """Newest version without a Start date → conflict still flagged but the
    proposal is None (validate-csv only hard-requires the column once a cut
    is CONFIGURED, so this shape is reachable)."""
    campaigns = [
        {"name": "app-search-phase2-v1", "clicks": 50, "conv": 0.0,
         "start_date": "2026-07-01"},
        {"name": "app-search-phase2-v2", "clicks": 20, "conv": 0.0},
    ]
    merged, _ = merge_ga_clicks(
        campaigns, [_mvp("app")], flag_version_conflicts=True
    )
    conflict = merged[0]["phase2_version_conflict"]
    assert conflict is not None
    assert conflict["proposed_relaunch_at"] is None


def test_validate_csv_phase2_relaunch_requires_start_date_column():
    """With a non-empty phase2 relaunch map, validate-csv hard-fails a CSV
    lacking the Start date column (campaign_passes_relaunch would otherwise
    fail closed and silently zero the configured MVP's denominator)."""
    with tempfile.TemporaryDirectory() as td:
        ga_csv_path = os.path.join(td, "ga.csv")
        cfg_path = os.path.join(td, "config.yaml")
        with open(ga_csv_path, "w", encoding="utf-8") as f:
            f.write("Campaign,Clicks\n")
            f.write("neuralpost-search-phase2-v2,30\n")
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write(
                "mvp_mappings:\n"
                "  neuralpost:\n    phase2_relaunch_at: '2026-08-01'\n"
        ...
        assert rc == 2
        # Same CSV passes when the map is empty (back-compat with the
        # Campaign+Clicks-only export).
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write("mvp_mappings: {}\n")
        rc = main([
            "validate-csv",
        ...
        assert rc == 0
        # And the Start-date-bearing CSV passes with the map set.
        with open(ga_csv_path, "w", encoding="utf-8") as f:
            f.write("Campaign,Clicks,Start date\n")
            f.write("neuralpost-search-phase2-v2,30,2026-08-01\n")
        with open(cfg_path, "w", encoding="utf-8") as f:
...
# ---------- ga_accounts (MCC sub-account attribution — ads-account owner channel) ----------

def test_merge_stamps_ga_accounts_on_matched_and_ga_only():
    mvps = [_mvp("diarly")]
    campaigns = [
        {"name": "diarly-search-v1", "clicks": 10, "conv": 0, "account": "Lew's MVP Account"},
        {"name": "diarly-search-v2", "clicks": 5, "conv": 0, "account": "Lew's MVP Account"},
        {"name": "reset-app-search-v1", "clicks": 58, "conv": 0, "account": "Radlin's MVP Account"},
    ]
    merged, _ = merge_ga_clicks(campaigns, mvps)
    by = {m["name"]: m for m in merged}
    assert by["diarly"]["ga_accounts"] == ["Lew's MVP Account"]  # deduped
    assert by["reset-app"]["ga_accounts"] == ["Radlin's MVP Account"]


def test_merge_ga_accounts_empty_cells_skipped_and_no_campaign_rows_default_empty():
    mvps = [_mvp("diarly"), _mvp("untouched")]
    campaigns = [{"name": "diarly-search-v1", "clicks": 3, "conv": 0, "account": ""}]
    merged, _ = merge_ga_clicks(campaigns, mvps)
    by = {m["name"]: m for m in merged}
    assert by["diarly"]["ga_accounts"] == []
    assert by["untouched"]["ga_accounts"] == []


def test_merge_orphan_via_ga_carries_accounts():
    """A campaign folding into an __orphan_* record stamps the account there —
    the ads-account owner channel's primary consumer (momrealm class)."""
    mvps = [_mvp("diarly"), _mvp("__orphan_momrealm__")]
    mvps[1]["orphan"] = True
    campaigns = [
        {"name": "momrealm-search-v1", "clicks": 82, "conv": 0, "account": "Lew's MVP Account"},
    ]
    merged, _ = merge_ga_clicks(campaigns, mvps)
    by = {m["name"]: m for m in merged}
    assert by["__orphan_momrealm__"]["ga_clicks"] == 82
    assert by["__orphan_momrealm__"]["ga_accounts"] == ["Lew's MVP Account"]


def test_merge_all_dropped_bucket_still_lands_accounts():
    """Account attribution is collected pre-relaunch-drop, like status."""
    mvps = [_mvp("mooncub")]
    campaigns = [{
        "name": "mooncub-search-v1", "clicks": 40, "conv": 0,
        "account": "Lee MVP", "start_date": "2026-05-01",
        "campaign_status": "Enabled", "serving_status": "Eligible",
    }]
    merged, unmatched = merge_ga_clicks(
        campaigns, mvps, relaunch_by_mvp={"mooncub": "2026-06-15"}
    )
    by = {m["name"]: m for m in merged}
    assert by["mooncub"]["ga_clicks"] == 0  # dropped pre-relaunch
    assert by["mooncub"]["ga_accounts"] == ["Lee MVP"]
    assert [u["reason"] for u in unmatched] == ["pre-relaunch"]
