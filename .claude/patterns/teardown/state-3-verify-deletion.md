...
- **Write verification artifact** (`.claude/runs/teardown-verification.json`):
  ```bash
  python3 -c "
  import json
  verification = {
  ...
  json.dump(verification, open('.claude/runs/teardown-verification.json', 'w'), indent=2)
  "
  ```

**VERIFY:**
```bash
test -f .claude/runs/teardown-verification.json
```
