...
| 4 | `visitors < thresholds.visitors_floor` (default 100) | `INSUFFICIENT_DATA` | Below visitors floor, can't conclude. Compute `visitors_needed = max(0, visitors_floor - visitors)`. **Early-pass escape (#2152):** when the numerator is DB-trusted (`signup_source` starts with `db_`) and `effective_signups >= ceil(conv_rate_go × visitors_floor)` with `visitors > 0`, the worst-case bound already clears the bar (rate ≥ conv_rate_go even if every remaining click converts at 0%) → `GO` with `metrics.early_pass = "worst_case_floor_bound"` + a `phase1_early_pass` info flag. PH-sourced numerators always wait for the floor (small-sample PH spikes may be attribution glitches — the dead-link case). |
