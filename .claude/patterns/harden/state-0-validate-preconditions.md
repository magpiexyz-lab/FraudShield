...
- If `.claude/runs/current-plan.md` exists AND the current branch starts with `chore/harden`:
  1. Read frontmatter. If parsing fails: stop -- "Plan file has corrupted frontmatter. Delete `.claude/runs/current-plan.md` and re-run `/harden` to start fresh."
...
- If on a `chore/harden-*` branch with existing specification tests but NO `.claude/runs/current-plan.md`: a previous `/harden` may have partially completed. Tell the user: "Found existing hardening work on this branch. Scanning for modules that still need tests..." Then scan for CRITICAL modules without test files and proceed from STATE 5 (module implementation loop), skipping STATE 4 if branch and config are already set up.

Create `.claude/runs/harden-context.json` to initialize state tracking:
```bash
cat > .claude/runs/harden-context.json << CTXEOF
...
- `.claude/runs/harden-context.json` exists
- Resume path determined (fresh start or checkpoint resume)

**VERIFY:**
```bash
test -f package.json && test -f .claude/runs/harden-context.json && echo "OK" || echo "FAIL"
```
