...
def test_postgres_lower_bound_expr_alias():
    # Default (no alias) is byte-identical to the historical form.
    assert (
        rl.postgres_lower_bound_expr("created_at", 90, None, alias="")
        == rl.postgres_lower_bound_expr("created_at", 90, None)
    )
    # Alias qualifies the column on both branches (join-ambiguity fix).
    plain = rl.postgres_lower_bound_expr("created_at", 90, None, alias="p")
    assert plain == "p.\"created_at\" >= now() - INTERVAL '90 days'"
    cut = rl.postgres_lower_bound_expr("created_at", 90, "2026-07-25", alias="p")
    assert cut.startswith('p."created_at" >= greatest(')
    assert "timestamptz '2026-07-25'" in cut


def test_posthog_per_mvp_relaunch_clause_empty():
    assert rl.posthog_per_mvp_relaunch_clause({}) == ("", {})
    assert rl.posthog_per_mvp_relaunch_clause(None) == ("", {})


def test_posthog_per_mvp_relaunch_clause_single():
    clause, values = rl.posthog_per_mvp_relaunch_clause({"neuralpost": "2026-08-01"})
    assert clause == (
        "AND NOT (properties.project_name = {rel_name_0} "
        "AND timestamp < toDateTime('2026-08-01 00:00:00')) "
    )
    assert values == {"rel_name_0": "neuralpost"}


def test_posthog_per_mvp_relaunch_clause_multi_sorted():
    clause, values = rl.posthog_per_mvp_relaunch_clause(
        {"perky": "2026-08-05", "neuralpost": "2026-08-01"}
    )
    # Sorted by name → deterministic placeholder assignment.
    assert values == {"rel_name_0": "neuralpost", "rel_name_1": "perky"}
    assert "toDateTime('2026-08-01 00:00:00')" in clause
    assert "toDateTime('2026-08-05 00:00:00')" in clause
    assert clause.index("{rel_name_0}") < clause.index("{rel_name_1}")


def test_posthog_per_mvp_relaunch_clause_rejects_garbage():
    try:
        rl.posthog_per_mvp_relaunch_clause({"neuralpost": "not-a-date"})
        raise AssertionError("expected ValueError")
    except ValueError:
        pass


def test_posthog_per_mvp_relaunch_clause_no_limit_placeholders():
    # The pagination shim str.formats templates containing {limit}/{offset};
    # the clause must never introduce them.
    clause, _ = rl.posthog_per_mvp_relaunch_clause({"a": "2026-08-01", "b": "2026-08-02"})
    assert "{limit}" not in clause and "{offset}" not in clause
